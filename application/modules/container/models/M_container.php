...
class M_container extends CI_Model {
	...
	$sql = "
		...
		A.HIST_ACTIVITY, A.HIST_CONT_STATUS, A.HIST_COUNTER, B.CONTAINER_SIZE CONTAINER_SIZE, NVL(A.HIST_SLOT_EXT,'-') HIST_SLOT_EXT, NVL(C.FULL_NAME,'System') HIST_USER, D.OWNER_NAME
		...
		LEFT JOIN TM_OWNER D ON D.OWNER_CODE = B.CONTAINER_OWNER
		WHERE
			HIST_CONT = ?
		AND HIST_BRANCH_ID = ?
		". $whereParams ."
	...
	public function getStuffingHistory($filter){
	    ...
	    //                 JOIN TM_CONSIGNEE C ON C.CONSIGNEE_ID = A.STUFF_CONSIGNEE_ID
			...
			$sql = "SELECT * FROM ( SELECT * FROM (
				 SELECT * FROM VIEW_STUFFING_PROCESS WHERE BRANCH_ID = ? $qWhere) T
							WHERE ROWNUM <= ?
					)
					WHERE  rnum >= ? + 1";
			...
			$sqlTotal = "SELECT COUNT(1) TOTAL FROM VIEW_STUFFING_PROCESS WHERE BRANCH_ID = ? $qWhere";
    ...
    //                 JOIN TM_CONSIGNEE C ON C.CONSIGNEE_ID = A.STRIP_CONSIGNEE_ID
		...
		// 		WHERE  rnum >= ? + 1";
		$sql = "SELECT * FROM ( SELECT * FROM (
			 SELECT * FROM VIEW_STRIPPING_PROCESS WHERE BRANCH_ID = ? $qWhere) T
						WHERE ROWNUM <= ?
				)
				WHERE  rnum >= ? + 1";
		$data = $this->db->query($sql,$params)->result();
		...
		$sqlTotal = "SELECT COUNT(1) TOTAL FROM VIEW_STRIPPING_PROCESS WHERE BRANCH_ID = ? $qWhere";
		$dataTotal = $this->db->query($sqlTotal,$paramsTotal)->row_array()['TOTAL'];
		return array (
	      'data' => $data,
	      'total' => $dataTotal
	...
	public function setContainerRename(){
		...
		$arrParam = $cont_old."~".$cont_new."~".$size_new."~".$type_new;
		//echo $arrParam;  die();
		...
		if($response->respon == 1){
				...
				if(count($cek_hist_cont) > 0){
					foreach ($cek_hist_cont as $val) {
						...
						if($activity == 3 AND ($activity_name == 'Receiving' || $activity_name == 'Placement')){
	...
	public function setContainerSize(){
		...
		$size_new = $this->input->post('SIZE');

		$this->db->trans_start();

		$cek_new_cont = $this->db->where('CONTAINER_NO',$cont)->where('CONTAINER_SIZE',$size_new)->where('CONTAINER_BRANCH_ID',$branch_id)->from('TM_CONTAINER')->count_all_results();

		if($cek_new_cont > 0){
			return array('success' => false, 'message' => 'Size yang anda pilih sama');
			die();
		}
		//Receiving
		$this->db->query("UPDATE TX_REQ_RECEIVING_DTL SET REQUEST_DTL_CONT_SIZE ='".$size_new."' WHERE REQUEST_DTL_CONT = '".$cont."'");

		//Delivery
		$this->db->query("UPDATE TX_REQ_DELIVERY_DTL SET REQ_DTL_CONT_SIZE ='".$size_new."' WHERE REQ_DTL_CONT = '".$cont."'");

		//Gate
		$this->db->query("UPDATE TX_GATE SET GATE_CONT_SIZE ='".$size_new."' WHERE GATE_CONT = '".$cont."'");

		//Placement
		$this->db->query("UPDATE TX_REAL_YARD SET REAL_YARD_CONT_SIZE ='".$size_new."' WHERE REAL_YARD_CONT = '".$cont."'");

		//Stuffing
		$this->db->query("UPDATE TX_REQ_STUFF_DTL SET STUFF_DTL_CONT_SIZE ='".$size_new."' WHERE STUFF_DTL_CONT = '".$cont."'");

		//Stripping
		$this->db->query("UPDATE TX_REQ_STRIP_DTL SET STRIP_DTL_CONT_SIZE ='".$size_new."' WHERE STRIP_DTL_CONT = '".$cont."'");

		//Master
		$this->db->query("UPDATE TM_CONTAINER SET CONTAINER_SIZE ='".$size_new."' WHERE CONTAINER_NO = '".$cont."'");

		$arrCont = array(
			'RENAMED_CONT_OLD' => $cont,
			'RENAMED_CONT_SIZE' => $size_new,
			'RENAMED_CREATE_BY' => $user_id,
			'RENAMED_BRANCH_ID' => $branch_id,
			'RENAMED_MARK' => 'Change Size'
		);
		$this->db->insert('TH_RENAMED',$arrCont);
