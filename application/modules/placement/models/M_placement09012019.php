		 ...
		 if($activity == 3){
			 if($origin == 'DEPO'){
			   $data = $this->db->select('A.GATE_ID ID, A.GATE_NOREQ REQ, A.GATE_CONT CONT, A.GATE_CONT_SIZE SIZE, A.GATE_CONT_TYPE TYPE, A.GATE_CONT_STATUS STATUS')
					...
					$recParams = array('CONT' => $cont, 'BRANCH' => $branch_id, 'ORIGIN' => $origin);
					$data = $this->db->query('SELECT B.REQUEST_ID ID, B.REQUEST_NO REQ, A.REQUEST_DTL_CONT CONT, A.REQUEST_DTL_CONT_SIZE "SIZE", A.REQUEST_DTL_CONT_TYPE TYPE, A.REQUEST_DTL_CONT_STATUS STATUS FROM TX_REQ_RECEIVING_DTL A
																		JOIN TX_REQ_RECEIVING_HDR B ON B.REQUEST_ID = A.REQUEST_HDR_ID
																		WHERE A.REQUEST_DTL_STATUS = 0 AND A.REQUEST_DTL_CONT = ? AND B.REQUEST_BRANCH_ID = ? AND B.REQUEST_FROM = ? ',$recParams)->row_array();
				}
			}
			else if ($activity == 2){
	...
	function setPlacement(){
	  ...
	  if($size == 40){
		...
		if($activity != 3){
				...
				// 					SELECT REAL_YARD_YBC_ID, REAL_YARD_TIER, REAL_YARD_BRANCH_ID, REAL_YARD_CONT, REAL_YARD_NO, ".$user_id." AS REAL_YARD_CREATE_BY, REAL_YARD_TYPE, 2 AS REAL_YARD_STATUS, REAL_YARD_ACTIVITY, '0' AS REAL_YARD_USED,
				...
				//
				// if($size == 40){
				// 	$cont_old40 = $this->db->query("SELECT * FROM (
				// 						SELECT REAL_YARD_YBC_ID, REAL_YARD_TIER, REAL_YARD_BRANCH_ID, REAL_YARD_CONT, REAL_YARD_NO, ".$user_id." AS REAL_YARD_CREATE_BY, REAL_YARD_TYPE, 2 AS REAL_YARD_STATUS, REAL_YARD_ACTIVITY, '0' AS REAL_YARD_USED,
				// 						REAL_YARD_REQ_NO, REAL_YARD_CONT_STATUS, TO_CHAR(REAL_YARD_MARK) REAL_YARD_MARK, REAL_YARD_CONT_SIZE, REAL_YARD_CONT_TYPE, REAL_YARD_VOY, REAL_YARD_VESSEL_CODE, REAL_YARD_VESSEL_NAME, REAL_YARD_COMMODITY
				// 						FROM TX_REAL_YARD A
				// 						WHERE A. REAL_YARD_BRANCH_ID = ".$branch_id."
				// 						AND A.REAL_YARD_YBC_ID = ".$ybc40."
				// 						AND A.REAL_YARD_ID IN (
				// 							SELECT X.REAL_YARD_ID  FROM (
				// 							 SELECT MAX(REAL_YARD_ID) REAL_YARD_ID FROM TX_REAL_YARD H WHERE H.REAL_YARD_YBC_ID = ".$ybc40." AND H.REAL_YARD_BRANCH_ID = ".$branch_id." GROUP BY H.REAL_YARD_CONT
				// 						 )X INNER JOIN TX_REAL_YARD I ON I.REAL_YARD_ID = X.REAL_YARD_ID WHERE I.REAL_YARD_STATUS = 1
				// 							)
				// 						)Z WHERE Z.REAL_YARD_CONT = '".$cont."'")->row_array();
				// 	$this->db->insert('TX_REAL_YARD',$cont_old40);
				// }
		...
		if($activity == 3){
			...
			//insert gate In & out
				...
				$insertGATEIN = "INSERT INTO TX_GATE (GATE_NOREQ, GATE_CONT, GATE_CONT_STATUS, GATE_CONT_SIZE, GATE_CONT_TYPE, GATE_TRUCK_NO, GATE_ORIGIN, GATE_CREATE_DATE, GATE_STATUS, GATE_ACTIVITY, GATE_STACK_STATUS, GATE_FL_SEND, GATE_BRANCH_ID) VALUES
										 ('".$req_id."','".$cont."','".$cont_status."','".$size."','".$type."','PELINDO','".$origin."', TO_DATE('".$gate_date_tpk."','MM/DD/YYYY HH24:MI:SS'),'1', '3','1', '9', ".$branch_id.")";
				$resultHDR = $this->db->query($insertGATEIN);

				$insertGATEIN = "INSERT INTO TX_GATE (GATE_NOREQ, GATE_CONT,GATE_CONT_STATUS, GATE_CONT_SIZE, GATE_CONT_TYPE, GATE_TRUCK_NO, GATE_ORIGIN, GATE_CREATE_DATE, GATE_STATUS, GATE_ACTIVITY, GATE_STACK_STATUS, GATE_FL_SEND, GATE_BRANCH_ID) VALUES
										 ('".$req_id."','".$cont."','".$cont_status."','".$size."','".$type."','PELINDO','".$origin."', TO_DATE('".$gate_date_tpk."','MM/DD/YYYY HH24:MI:SS'),'3', '3','2', '9', ".$branch_id.")";
				$resultHDR = $this->db->query($insertGATEIN);
