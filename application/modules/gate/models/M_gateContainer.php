...
class M_gateContainer extends CI_Model {
  ...
  function setGateInContainer() {
	...
	function setGateOutContainer(){
		...
		//cek jika tgl backdate delivery melebihi tanggal expired
		if($this->input->post('ACTIVITY') == 4){
			if(isset($_POST['GATE_IN_DATE']) && $_POST['GATE_IN_DATE'] != 'Date' && isset($_POST['GATE_IN_TIME'])) {
				$arrBackDate = explode('/',$_POST['GATE_IN_DATE']);
				$date_gato = $arrBackDate[2].'/'.$arrBackDate[1].'/'.$arrBackDate[0];
				$date_gato = strtotime($date_gato);
			}
			else{
				$date_gato = strtotime(date('Y/m/d'));
			}

			$del_paid_thru = $this->db->select("TO_CHAR(A.REQ_DTL_DEL_DATE,'YYYY/MM/DD') PAID_THRU")
							 ->from('TX_REQ_DELIVERY_DTL A')
							 ->join('TX_REQ_DELIVERY_HDR B','B.REQ_ID = A.REQ_HDR_ID')
							 ->where('A.REQ_DTL_ACTIVE','Y')
							 ->where('A.REQ_DTL_CONT',$this->input->post('CONT_NO'))
							 ->get()->row_array()['PAID_THRU'];
			$del_paid_thru = strtotime($del_paid_thru);

			if($date_gato > $del_paid_thru){
				return array('success' => false, 'message' => 'Delivery Expired, silahkan melalukan perpanjangan untuk GATE OUT container ini.');
				die();
			}
		}
	...
	function rePrintGate(){
		...
		if($gate['GATE_ACTIVITY'] == 3){
			...
			$cont_rec = $this->db->select("A.REQUEST_DTL_CONT, A.REQUEST_DTL_CONT_SIZE, A.REQUEST_DTL_CONT_TYPE, B.REQUEST_DI DI, A.REQUEST_DTL_CONT_STATUS, A.REQUEST_DTL_OWNER_CODE, NVL(A.REQUEST_DTL_OWNER_NAME,'-') REQUEST_DTL_OWNER_NAME")
									->from('TX_REQ_RECEIVING_DTL A')
				...
				//crate array for print
					...
					$owner_rec = $cont_rec['REQUEST_DTL_OWNER_NAME'];
		...
		else
		{
			...
			$cont_in_yard = $this->db->query("SELECT * FROM(SELECT MAX(REAL_YARD_ID) REAL_YARD_ID, REAL_YARD_CONT, YARD, BLOCK_, SLOT_, ROW_, TIER_, BLOCK_ID  FROM(
					...
					$owner_delivery = $this->db->query("SELECT NVL(B.OWNER_NAME,'-') OWNER_NAME FROM TM_CONTAINER A LEFT JOIN TM_OWNER B ON B.OWNER_CODE = A.CONTAINER_OWNER WHERE A.CONTAINER_NO = '".$gate['GATE_CONT']."' AND A.CONTAINER_BRANCH_ID = ".$branch_id)->row_array()['OWNER_NAME'];
		 ...
		 $status_check = $this->db->select('A.CAT_DTL_CONT_STATUS')->from('TX_CATEGORY_DTL A')->join('TX_CATEGORY_HDR B','B.CAT_HDR_ID = A.CAT_HDR_ID')->join('TX_YARD_PLAN_GROUP C','C.YPG_CAT_HDR_ID = B.CAT_HDR_ID')
	 	 ...
	 	 $di_check = $this->db->select('A.CAT_DTL_EXIM')->from('TX_CATEGORY_DTL A')->join('TX_CATEGORY_HDR B','B.CAT_HDR_ID = A.CAT_HDR_ID')->join('TX_YARD_PLAN_GROUP C','C.YPG_CAT_HDR_ID = B.CAT_HDR_ID')
			...
			$lokasi = $this->db->query("SELECT B.CAT_HDR_ID, A.CAT_DTL_CONT_SIZE CONT, A.CAT_DTL_CONT_STATUS, A.CAT_DTL_CONT_TYPE, A.CAT_DTL_EXIM, C.YPG_YARD_ID, C.YPG_BLOCK_ID, C.YPG_STAR_ROW, C.YPG_END_ROW, C.YPG_START_SLOT, C.YPG_END_SLOT, C.YPG_CAPACITY, A.CAT_DTL_EXIM DI, D.BLOCK_NAME, E.YARD_NAME
										...
										WHERE ".$where." ORDER BY YPG_START_SLOT, YPG_STAR_ROW ASC")->result_array();
