          ...
          for(var i=0;i<resData.data[key].length;i++){
              if(resData.data[key][i].CONT_SIZE == 20){
                if(resData.data[key][i].DELIVERY_TO == 'DEPO'){
                  ...
                  ).addClass('delivery_luar').css({"position":"relative"} );
                }
                else if(resData.data[key][i].DELIVERY_TO == 'TPK'){
                  $(".ciy_table[data-slot="+key+"] td[data-row="+resData.data[key][i].YD_ROW+"][data-tier="+resData.data[key][i].YD_TIER+"]").html(
                      generateDataContainer(resData.data[key][i])
                  ).addClass('delivery_tpk').css({"position":"relative"} );
                }
                else{
                  $(".ciy_table[data-slot="+key+"] td[data-row="+resData.data[key][i].YD_ROW+"][data-tier="+resData.data[key][i].YD_TIER+"]").html(
                      generateDataContainer(resData.data[key][i])
    ...
    function tierMonitoring(branch, ybc_id, tier, cont, loc, ybc_id40, size, ybc_id_old, ybc_id_old40, tier_old, tier_old40, detail, delivery_to){
      if (branch == Ext.util.Cookies.get('branch')) {
        ...
        if(ybc_id_old != 0){
          ...
          $('td[data-block='+detail.old_loc.BLOCK_ID+'][data-slot='+detail.old_loc.YBC_SLOT+'][data-row='+detail.old_loc.YBC_ROW+'][data-tier='+detail.tier_old+']').removeClass('stacking stacking40 delivery_luar delivery_tpk');
        ...
        $('td[data-block='+detail.new_loc.BLOCK_ID+'][data-slot='+detail.new_loc.YBC_SLOT+'][data-row='+detail.new_loc.YBC_ROW+'][data-tier='+detail.tier+']').removeClass('stacking stacking40 delivery_luar delivery_tpk');
        ...
        if(tier > 0){
          ...
          if(size == 40){
            if(delivery_to == 'TPK'){
              $('td[data-block='+detail.new_loc.BLOCK_ID+'][data-slot='+detail.new_loc.YBC_SLOT+'][data-row='+detail.new_loc.YBC_ROW+'][data-tier='+tier+']').addClass('delivery_tpk');
            }
            else if(delivery_to == 'DEPO'){
              $('td[data-block='+detail.new_loc.BLOCK_ID+'][data-slot='+detail.new_loc.YBC_SLOT+'][data-row='+detail.new_loc.YBC_ROW+'][data-tier='+tier+']').addClass('delivery_luar');
            }
            else{
