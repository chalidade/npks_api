                     ...
                     //if($cont_size_real[$p] == 40 ){echo "fourtyfeet "; }
                      ...
                      if($block_dummy[$p] == 'N'){
                        ...
                        echo "dummy";
									...
									?> <?php if($cont_size[$p] == 40 && $placement[$p] == 0) {echo 'fourtyfeet_background'; } ?>"
...
$(".exist").on('click', function(event){
          ...
          filterContent += '</tr></table><br>';
              ...
              if (!ux_singlestackview){
                  ...
                  ux_singlestackview = Ext.create('Ext.Window', {
                      title: 'Container in Yard View 1',
                      autoScroll: true,
                      scrollable: true,
                      width: w,
                      //maxWidth: '80%',
      ...
      for(var key in resData.data){
          for(var i=0;i<resData.data[key].length;i++){
              if(resData.data[key][i].CONT_SIZE == 20){
                ...
                ).addClass('stacking').css({"position":"relative"} );
              }
              else{
                $(".ciy_table[data-slot="+key+"] td[data-row="+resData.data[key][i].YD_ROW+"][data-tier="+resData.data[key][i].YD_TIER+"]").html(
                    generateDataContainer(resData.data[key][i])
                ).addClass('stacking40').css({"position":"relative"} );
              }
          }
      }
  }
  ...
  function generateDataContainer(dataContainer){
      ...
      var data = htmlDiv + header + '<br />'+dtl+ '<span class="CellComment">No Container : '+cont+'<br> Size : '+size+'<br> Type : '+type+'<br> Status : '+status+'</span>';
      return data;
  }
  ...
  if(Globals.getSocketYardMon() == 0){
    ...
    function tierMonitoring(branch, ybc_id, tier, cont, loc, ybc_id40, size, ybc_id_old, ybc_id_old40, tier_old, tier_old40, detail){
      if (branch == Ext.util.Cookies.get('branch')) {
        ...
        if(ybc_id_old != 0){
          ...
          $('td[data-block='+detail.old_loc.BLOCK_ID+'][data-slot='+detail.old_loc.YBC_SLOT+'][data-row='+detail.old_loc.YBC_ROW+'][data-tier='+detail.tier_old+']').removeClass('stacking stacking40');
        ...
        $('td[data-block='+detail.new_loc.BLOCK_ID+'][data-slot='+detail.new_loc.YBC_SLOT+'][data-row='+detail.new_loc.YBC_ROW+'][data-tier='+detail.tier+']').removeClass('stacking stacking40');
        ...
        if(tier > 0){
          ...
          if(size == 40){
            $('td[data-block='+detail.new_loc.BLOCK_ID+'][data-slot='+detail.new_loc.YBC_SLOT+'][data-row='+detail.new_loc.YBC_ROW+'][data-tier='+tier+']').addClass('stacking40');
          }
          else{
            $('td[data-block='+detail.new_loc.BLOCK_ID+'][data-slot='+detail.new_loc.YBC_SLOT+'][data-row='+detail.new_loc.YBC_ROW+'][data-tier='+tier+']').addClass('stacking');
          }
        ...
        if(size == 40){
          if(ybc_id_old40 > 0){
            ...
            $('td[data-block='+detail.old_loc40.BLOCK_ID+'][data-slot='+detail.old_loc40.YBC_SLOT+'][data-row='+detail.old_loc40.YBC_ROW+'][data-tier='+detail.tier_old+']').removeClass('stacking stacking40');
          ...
          $('td[data-block='+detail.new_loc40.BLOCK_ID+'][data-slot='+detail.new_loc40.YBC_SLOT+'][data-row='+detail.new_loc40.YBC_ROW+'][data-tier='+detail.tier+']').removeClass('stacking stacking40');
                  ...
                  filterContent += '</tr></table><br>';
                      ...
                      if (!ux_singlestackview_){
                          ...
                          ux_singlestackview_ = Ext.create('Ext.Window', {
                              title: 'Container in Yard View 2',
                              autoScroll: true,
                              scrollable: true,
                              width: w,
                              //maxWidth: '80%',
