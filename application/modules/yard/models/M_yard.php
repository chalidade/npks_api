...
class M_yard extends CI_Model {
    ...
    foreach ($block as $block_){
      ...
      $query_yard_block = "INSERT INTO TM_BLOCK(BLOCK_YARD_ID, BLOCK_NAME, BLOCK_ROW, BLOCK_SLOT, BLOCK_TIER, BLOCK_POSITION, BLOCK_ORIENTATION, BLOCK_BRANCH_ID, BLOCK_CAPACITY, BLOCK_DUMMY)
                           VALUES($yard_id, '$block_->name', $block_row, $block_slot, $block_->tier, '$block_->position', '$block_->orientation', $branch_id, $block_capacity, '$block_dummy')";
  ...
  public function set_block_bl_v($yard_id, $block_id, $arrCell, $row, $slot){
	...
	$blockData = $this->db->select('BLOCK_ROW, BLOCK_SLOT')->from('TM_BLOCK')->where('BLOCK_YARD_ID',$yard_id)->where('BLOCK_ID',$block_id)->get()->row_array();
	$slot = (int)$blockData['BLOCK_SLOT'];
	$row = (int)$blockData['BLOCK_ROW'];
	$tier = (int)$this->input->post('BLOCK_TIER');
	$capacity = ($slot * $row) * $tier;
...
function getPlanCategory(){
	...
	$filter = "";
	if($query){
		$filter = "AND CAT_HDR_NAME LIKE '%".$this->db->escape_like_str($query)."%' ESCAPE '!'";
	}
	$query = "SELECT CAT_HDR_ID, CAT_HDR_NAME FROM TX_CATEGORY_HDR WHERE CAT_BRANCH_ID = ? $filter";
	return $this->db->query($query,$params)->result_array();
}
...
function getYardTier(){
	...
	$real_tier = $this->db->query("SELECT MAX(Z.REAL_YARD_TIER) LAST_TIER FROM (SELECT I.REAL_YARD_TIER  FROM (
									 ...
									 )X INNER JOIN TX_REAL_YARD I ON I.REAL_YARD_ID = X.REAL_YARD_ID WHERE I.REAL_YARD_STATUS = 1 AND I.REAL_YARD_USED = 1
...
// 		}
// 	}
	// return max($tierActive);
	return array(array('BLOCK_TIER' => $real_tier, 'MAX_TIER' => max($tierActive)));
}
...
$params = array(
	'YARD_ID' => $id_yard,
	'BRANCH_ID' => $this->session->USER_BRANCH,
	'BRANCH_ID2' => $this->session->USER_BRANCH
);
$xml_str = "";
$query 		= "SELECT * FROM TM_YARD WHERE YARD_ID='$id_yard'";
$rs 		= $this->db->query($query);
...
// $query 		= "SELECT T1.*, (SELECT MAX(G.CAT_DTL_CONT_SIZE) FROM TX_YARD_PLAN_GROUP E
// 			 	LEFT JOIN TX_CATEGORY_HDR F ON F.CAT_HDR_ID = E.YPG_CAT_HDR_ID
// 			 	LEFT JOIN TX_CATEGORY_DTL G ON G.CAT_HDR_ID = F.CAT_HDR_ID WHERE
// 			 	E.YPG_YARD_ID = T1.YBC_YARD_ID AND E.YPG_BLOCK_ID = T1.BLOCK_ID  GROUP BY E.YPG_YARD_ID, E.YPG_BLOCK_ID) CAT_DTL_CONT_SIZE
// 				FROM (
// 					SELECT A.YBC_YARD_ID ,A.YBC_CELL_ID, B.BLOCK_ID, B.BLOCK_NAME, B.BLOCK_POSITION, B.BLOCK_ORIENTATION, B.BLOCK_TIER, A.YBC_ROW, A.YBC_SLOT, B.BLOCK_ROW MAX_ROW, B.BLOCK_SLOT MAX_SLOT, A.YBC_BRANCH_ID, E.OWNER_NAME,
// 						MAX(C.YP_STATUS) FLAG_STATUS,SUM(DECODE(NVL(C.YP_STATUS,0),0,0,1)) JML_TAKEN, 0 PLACEMENT, NVL(FUNC_GET_SIZE_REAL(A.YBC_ID, A.YBC_BRANCH_ID),0) CONT_SIZE_REAL, NVL(FUNC_GET_TOT_STACK(A.YBC_ID, A.YBC_BRANCH_ID),0) JML_PLACEMENT, B.BLOCK_ROW, B.BLOCK_SLOT, D.YARD_HEIGHT, D.YARD_WIDTH, A.YBC_ID, B.BLOCK_DUMMY
// 						FROM TX_YARD_BLOCK_CELL A
// 						JOIN TM_BLOCK B ON A.YBC_BLOCK_ID = B.BLOCK_ID
// 						LEFT JOIN TX_YARD_PLAN C ON A.YBC_ID = C.YP_YBC_ID
// 						JOIN TM_YARD D ON B.BLOCK_YARD_ID = D.YARD_ID
// 						LEFT JOIN VIEW_OWNER_PLAN_GROUP E ON E.CAT_HDR_ID = C.YP_CAT_HDR_ID
// 						WHERE A.YBC_ACTIVE = 'Y' AND B.BLOCK_ACTIVE = 'Y' AND A.YBC_YARD_ID = ? AND A.YBC_BRANCH_ID = ?
// 						GROUP BY A.YBC_YARD_ID ,A.YBC_CELL_ID, B.BLOCK_ID, B.BLOCK_NAME, B.BLOCK_POSITION, B.BLOCK_ORIENTATION, B.BLOCK_TIER, A.YBC_ROW, A.YBC_SLOT, B.BLOCK_ROW, B.BLOCK_SLOT, A.YBC_BRANCH_ID, D.YARD_HEIGHT, D.YARD_WIDTH, A.YBC_ID, B.BLOCK_DUMMY, E.OWNER_NAME
//
// 				) T1 WHERE T1.YBC_BRANCH_ID = ?
// 				ORDER BY T1.YBC_CELL_ID";
...
// 			 	E.YPG_YARD_ID = T1.YBC_YARD_ID AND E.YPG_BLOCK_ID = T1.BLOCK_ID  GROUP BY E.YPG_YARD_ID, E.YPG_BLOCK_ID) CAT_DTL_CONT_SIZE
// 				FROM (
// 					SELECT A.YBC_YARD_ID ,A.YBC_CELL_ID, B.BLOCK_ID, B.BLOCK_NAME, B.BLOCK_POSITION, B.BLOCK_ORIENTATION, B.BLOCK_TIER, A.YBC_ROW, A.YBC_SLOT, B.BLOCK_ROW MAX_ROW, B.BLOCK_SLOT MAX_SLOT, A.YBC_BRANCH_ID, E.CAT_HDR_NAME OWNER_NAME,
// 						MAX(C.YP_STATUS) FLAG_STATUS,SUM(DECODE(NVL(C.YP_STATUS,0),0,0,1)) JML_TAKEN, 0 PLACEMENT, NVL(FUNC_GET_SIZE_REAL(A.YBC_ID, A.YBC_BRANCH_ID),0) CONT_SIZE_REAL, NVL(FUNC_GET_TOT_STACK(A.YBC_ID, A.YBC_BRANCH_ID),0) JML_PLACEMENT, B.BLOCK_ROW, B.BLOCK_SLOT, D.YARD_HEIGHT, D.YARD_WIDTH, A.YBC_ID, B.BLOCK_DUMMY
// 						FROM TX_YARD_BLOCK_CELL A
// 						JOIN TM_BLOCK B ON A.YBC_BLOCK_ID = B.BLOCK_ID
// 						LEFT JOIN TX_YARD_PLAN C ON A.YBC_ID = C.YP_YBC_ID
// 						JOIN TM_YARD D ON B.BLOCK_YARD_ID = D.YARD_ID
// 						--LEFT JOIN VIEW_OWNER_PLAN_GROUP E ON E.CAT_HDR_ID = C.YP_CAT_HDR_ID
// 						LEFT JOIN TX_CATEGORY_HDR E ON E.CAT_HDR_ID = C.YP_CAT_HDR_ID
// 						WHERE A.YBC_ACTIVE = 'Y' AND B.BLOCK_ACTIVE = 'Y' AND A.YBC_YARD_ID = ? AND A.YBC_BRANCH_ID = ?
// 						GROUP BY A.YBC_YARD_ID ,A.YBC_CELL_ID, B.BLOCK_ID, B.BLOCK_NAME, B.BLOCK_POSITION, B.BLOCK_ORIENTATION, B.BLOCK_TIER, A.YBC_ROW, A.YBC_SLOT, B.BLOCK_ROW, B.BLOCK_SLOT, A.YBC_BRANCH_ID, D.YARD_HEIGHT, D.YARD_WIDTH, A.YBC_ID, B.BLOCK_DUMMY, E.CAT_HDR_NAME
//
// 				) T1 WHERE T1.YBC_BRANCH_ID = ?
// 				ORDER BY T1.YBC_CELL_ID";
...
$query 		= "SELECT T1.*, (SELECT MAX(G.CAT_DTL_CONT_SIZE) FROM TX_YARD_PLAN_GROUP E
 			 	...
 			 	E.YPG_YARD_ID = T1.YBC_YARD_ID AND E.YPG_BLOCK_ID = T1.BLOCK_ID  GROUP BY E.YPG_YARD_ID, E.YPG_BLOCK_ID) CAT_DTL_CONT_SIZE
 				FROM (
 					SELECT A.YBC_YARD_ID ,A.YBC_CELL_ID, B.BLOCK_ID, B.BLOCK_NAME, B.BLOCK_POSITION, B.BLOCK_ORIENTATION, B.BLOCK_TIER, A.YBC_ROW, A.YBC_SLOT, B.BLOCK_ROW MAX_ROW, B.BLOCK_SLOT MAX_SLOT, A.YBC_BRANCH_ID, LISTAGG (E.CAT_HDR_NAME,'/') WITHIN GROUP (ORDER BY C.YP_YBC_ID) OWNER_NAME,
 						MAX(C.YP_STATUS) FLAG_STATUS,SUM(DECODE(NVL(C.YP_STATUS,0),0,0,1)) JML_TAKEN, 0 PLACEMENT, NVL(FUNC_GET_SIZE_REAL(A.YBC_ID, A.YBC_BRANCH_ID),0) CONT_SIZE_REAL, NVL(FUNC_GET_TOT_STACK(A.YBC_ID, A.YBC_BRANCH_ID),0) JML_PLACEMENT, B.BLOCK_ROW, B.BLOCK_SLOT, D.YARD_HEIGHT, D.YARD_WIDTH, A.YBC_ID, B.BLOCK_DUMMY
 						FROM TX_YARD_BLOCK_CELL A
 						JOIN TM_BLOCK B ON A.YBC_BLOCK_ID = B.BLOCK_ID
 						LEFT JOIN TX_YARD_PLAN C ON A.YBC_ID = C.YP_YBC_ID
 						...
 						LEFT JOIN TX_CATEGORY_HDR E ON E.CAT_HDR_ID = C.YP_CAT_HDR_ID
 						WHERE A.YBC_ACTIVE = 'Y' AND B.BLOCK_ACTIVE = 'Y' AND A.YBC_YARD_ID = ? AND A.YBC_BRANCH_ID = ?
 						GROUP BY A.YBC_YARD_ID ,A.YBC_CELL_ID, B.BLOCK_ID, B.BLOCK_NAME, B.BLOCK_POSITION, B.BLOCK_ORIENTATION, B.BLOCK_TIER, A.YBC_ROW, A.YBC_SLOT, B.BLOCK_ROW, B.BLOCK_SLOT, A.YBC_BRANCH_ID, D.YARD_HEIGHT, D.YARD_WIDTH, A.YBC_ID, B.BLOCK_DUMMY
 				) T1 WHERE T1.YBC_BRANCH_ID = ?
 				ORDER BY T1.YBC_CELL_ID";
...
$index_label_text = array();
$index_ybc_id = array();
$index_cont_size = array();
$index_cont_size_real = array();
$max_row = array();
$max_slot = array();
$index_block_dummy = array();
$index_owner = array();
...
for ($e=0; $e < $total_cell; $e++) {
	...
	foreach($data as $row){
		...
		if($row['YBC_CELL_ID'] == $e){
			...
			$index_cont_size[] = $row['CAT_DTL_CONT_SIZE'];
			$index_cont_size_real[] = $row['CONT_SIZE_REAL'];
			...
			$delta = $row['JML_TAKEN']-$row['JML_PLACEMENT'];
			$index_taken[] = $delta;
			$index_block_dummy[] = $row['BLOCK_DUMMY'];
			$index_owner[] = $row['OWNER_NAME'];
...
$cont_size_ = implode(",",$index_cont_size);
$cont_size_real_ = implode(",",$index_cont_size_real);
$ybc_id_ = implode(",",$index_ybc_id);
$stack_ = implode(",",$index_stack);
$plan_ = implode(",",$index_plan);
$taken_ = implode(",",$index_taken);
...
$label_ = implode(",",$index_label);
$label_text_ = implode(",",$index_label_text);
$block_dummy_ = implode(",",$index_block_dummy);
$owner_ = implode(",",$index_owner);

$cont_size_str = "<cont_size>".$cont_size_."</cont_size>";
$cont_size_real_str = "<cont_size_real>".$cont_size_real_."</cont_size_real>";
$ybc_id_str = "<ybc_id>".$ybc_id_."</ybc_id>";
$stack_str = "<index>".$stack_."</index>";
$plan_str = "<plan>".$plan_."</plan>";
$taken_str = "<taken>".$taken_."</taken>";
...
$label_str = "<label>".$label_."</label>";
$label_text_str = "<label_text>".$label_text_."</label_text>";
$label_block_dummy = "<block_dummy>".$block_dummy_."</block_dummy>";
$label_owner = "<owner>".$owner_."</owner>";

$xml_str = "<yard>".$width_str.$height_str.$stack_str.$plan_str.$taken_str.$placement_str.$slot_str.$max_slot_str.$row_str.$max_row_str.$tier_str.$title_str.$block_id_str.$orientation_str.$position_str.$label_str.$label_text_str.$ybc_id_str.$cont_size_str.$label_block_dummy.$cont_size_real_str.$label_owner."</yard>";
return $xml_str;
}
...
public function get_yard_stacking($id_yard, $block, $slot, $ybc_id){
  ...
  // SELECT C.BLOCK_NAME, B.YBC_ROW, B.YBC_SLOT, A.REAL_YARD_TIER, A.REAL_YARD_CONT, A.REAL_YARD_CONT_SIZE, A.REAL_YARD_CONT_TYPE, A.REAL_YARD_CONT_STATUS, A.REAL_YARD_COMMODITY, A.REAL_YARD_STATUS, A.REAL_YARD_CREATE_DATE, A.REAL_YARD_YBC_ID
  // FROM TX_REAL_YARD A
  ...
  //    SELECT MAX(REAL_YARD_ID) REAL_YARD_ID FROM TX_REAL_YARD H WHERE H.REAL_YARD_YBC_ID IN(SELECT YBC_ID FROM TX_YARD_BLOCK_CELL WHERE YBC_BLOCK_ID = ".$block." AND YBC_SLOT = ".$slot." AND YBC_ACTIVE = 'Y') GROUP BY H.REAL_YARD_CONT, H.REAL_YARD_YBC_ID
  //  )X INNER JOIN TX_REAL_YARD I ON I.REAL_YARD_ID = X.REAL_YARD_ID WHERE I.REAL_YARD_STATUS = 1 AND I.REAL_YARD_USED = 1
  //   )
  // )Z")->result_array();

	// $query2 = $this->db->query("SELECT C.BLOCK_NAME, B.YBC_ROW, B.YBC_SLOT, A.REAL_YARD_TIER, A.REAL_YARD_CONT, A.REAL_YARD_CONT_SIZE, A.REAL_YARD_CONT_TYPE, A.REAL_YARD_CONT_STATUS, A.REAL_YARD_COMMODITY, A.REAL_YARD_STATUS, A.REAL_YARD_CREATE_DATE, A.REAL_YARD_YBC_ID
	// FROM TX_REAL_YARD A
	// JOIN TX_YARD_BLOCK_CELL B ON B.YBC_ID = A.REAL_YARD_YBC_ID
	// JOIN TM_BLOCK C ON C.BLOCK_ID = B.YBC_BLOCK_ID
	// WHERE A. REAL_YARD_BRANCH_ID = ".$branch_id." AND B.YBC_ACTIVE = 'Y' AND C.BLOCK_ACTIVE = 'Y' AND B.YBC_BLOCK_ID = ".$block." AND B.YBC_SLOT = ".$slot." AND A.REAL_YARD_USED = 1 "
	// )->result_array();

	$query2 = $this->db->query("SELECT C.BLOCK_NAME, B.YBC_ROW, B.YBC_SLOT, A.REAL_YARD_TIER, A.REAL_YARD_CONT, A.REAL_YARD_CONT_SIZE, A.REAL_YARD_CONT_TYPE, A.REAL_YARD_CONT_STATUS, A.REAL_YARD_COMMODITY, A.REAL_YARD_STATUS, A.REAL_YARD_CREATE_DATE, A.REAL_YARD_YBC_ID, NVL(E.REQUEST_TO,'T') DELIVERY_TO, NVL(G.OWNER_NAME,'-') OWNER_NAME
	FROM TX_REAL_YARD A
	JOIN TX_YARD_BLOCK_CELL B ON B.YBC_ID = A.REAL_YARD_YBC_ID
	JOIN TM_BLOCK C ON C.BLOCK_ID = B.YBC_BLOCK_ID
	LEFT JOIN TX_REQ_DELIVERY_DTL D ON D.REQ_DTL_CONT = A.REAL_YARD_CONT AND D.REQ_DTL_STATUS <> 2
	LEFT JOIN TX_REQ_DELIVERY_HDR E ON E.REQ_ID = D.REQ_HDR_ID
	LEFT JOIN TM_CONTAINER F ON F.CONTAINER_NO = A.REAL_YARD_CONT
	LEFT JOIN TM_OWNER G ON G.OWNER_CODE = F.CONTAINER_OWNER
	WHERE A. REAL_YARD_BRANCH_ID = ".$branch_id." AND B.YBC_ACTIVE = 'Y' AND C.BLOCK_ACTIVE = 'Y' AND B.YBC_BLOCK_ID = ".$block." AND B.YBC_SLOT = ".$slot." AND A.REAL_YARD_USED = 1 "
	)->result_array();
...
public function get_block_select($id_yard, $block){
	  $branch_id = $this->session->USER_BRANCH;
		$data = $this->db->select('A.BLOCK_NAME, A.BLOCK_ID')
						->from('TM_BLOCK A')
						->join('TM_YARD B','B.YARD_ID = A.BLOCK_YARD_ID')
						->where('A.BLOCK_YARD_ID',$id_yard)
	...
	function planOwner(){
		...
		$data = $this->db->where('OWNER_BRANCH_ID',$branch_id)->like('OWNER_NAME',$filter)->get('TM_OWNER')->result_array();
		return $data;
	}
		...
		$sql = "SELECT * FROM
				(
					SELECT TABLE_1.*, rownum AS rnum FROM (
						...
						WHERE C.BLOCK_ACTIVE = 'Y' AND A.YPG_BRANCH_ID = ? AND YPG_YARD_ID = ".$YARD_ID."
	...
	public function reportYor(){
		...
		$yard = (int)$_REQUEST['yard'];
		$date1 = $_REQUEST['date1'] != null? date('dmY',strtotime(substr($_REQUEST['date1'],0,-9))) : date('dmY');
		$date2 = $_REQUEST['date1'] != null? date('mY',strtotime(substr($_REQUEST['date1'],0,-9))) : date('mY');
		$date3 = $_REQUEST['date1'] != null? date('Y-m-d',strtotime(substr($_REQUEST['date1'],0,-9))) : date('Y-m-d');
		// $date4 = $_REQUEST['date1'] != null? date('Y-m-d',strtotime(substr($_REQUEST['date1'],0,-11).'01')) : date('Y-m-d');
		$date_select =  $_REQUEST['date1'] != null? date('d',strtotime(substr($_REQUEST['date1'],0,-9))) : date('dmY');
		$yesterday =  date('dmY',strtotime($date3.'- 1 days'));

		$this->db->query("CALL PROC_REPORT_YOR(
			 '".$date1."',
			 ".$yard.",
			 ".$branch_id.",
			 ".$user_id."
		...
		$data = $this->db->select("A.*, TO_CHAR(YOR_DATE,'DD/MM/YYYY') YOR_DATE2")->where('YOR_BRANCH_ID',$branch_id)->where("to_char(YOR_DATE,'DDMMYYYY')","'$date1'",false)->where('YOR_YARD_ID',$yard)->order_by("TO_CHAR(YOR_DATE,'YYYYMMDD')")->get('TH_REPORT_YOR A')->row_array();
		$hariSebelumnya = $this->db->where('YOR_BRANCH_ID',$branch_id)->where("YOR_DATE","to_date('$yesterday','DDMMYYYY')",false)->where('YOR_YARD_ID',$yard)->order_by("TO_CHAR(YOR_DATE,'YYYYMMDD')")->get('TH_REPORT_YOR')->row_array();
		$total_box_stack = $this->db->distinct()->select('REAL_YARD_CONT')->from('TX_REAL_YARD')->where('REAL_YARD_BRANCH_ID',$branch_id)->where('REAL_YARD_NO',$yard)->where('REAL_YARD_STATUS',1)->where('REAL_YARD_USED',1)->count_all_results();
		$total_teus_stack = $this->db->select('COUNT(REAL_YARD_ID) REAL_YARD_ID')->from('TX_REAL_YARD')->where('REAL_YARD_BRANCH_ID',$branch_id)->where('REAL_YARD_NO',$yard)->where('REAL_YARD_STATUS',1)->where('REAL_YARD_USED',1)->get()->row_array()['REAL_YARD_ID'];
		...
		$ytp = $this->db->where('YBC_YARD_ID',$yard)->where('YBC_BRANCH_ID',$branch_id)->from('TX_YARD_BLOCK_CELL')->count_all_results();
		...
		$lastteus =0;
			...
			if($date_select == 01){
				if($yesterday == '30112018'){
					$box = 0;
					$teus = 0;
					$total_ytp = 0;
				}
				else{
					$box = (int)$hariSebelumnya['YOR_BOX'] + (int)$hariSebelumnya['YOR_REC_BOX'] - (int)$hariSebelumnya['YOR_DEL_BOX'];
					$teus = (int)$hariSebelumnya['YOR_TEUS'] + (int)$hariSebelumnya['YOR_REC_TEUS'] - (int)$hariSebelumnya['YOR_DEL_TEUS'];
					$total_ytp = (int)$hariSebelumnya['YOR_REC_TEUS'] / (int)$ytp;
				}

			}
			else{
				$box = (int)$hariSebelumnya['YOR_BOX'] + (int)$data['YOR_REC_BOX'] - (int)$data['YOR_DEL_BOX'];
				$teus = (int)$hariSebelumnya['YOR_TEUS'] + (int)$data['YOR_REC_TEUS'] - (int)$data['YOR_DEL_TEUS'];
				$total_ytp = (int)$data['YOR_REC_TEUS'] / (int)$ytp;
			}

			$total_box = (int)$box;
			$total_teus = (int)$teus;

			$yor = round(($total_teus/$total_yor)*100,2);
			$ytp = round($total_ytp,2);
			$yordate = date('dmY',strtotime($data['YOR_DATE']));
			$this->db->query("UPDATE TH_REPORT_YOR SET YOR_BOX = $total_box, YOR_TEUS = $total_teus, YOR_TOTAL = $yor, YTP_TOTAL = $ytp WHERE YOR_BRANCH_ID = $branch_id AND TO_CHAR(YOR_DATE,'DDMMYYYY') = '$yordate' ");

			$lastbox =  $total_box;
			$lastteus =  $total_teus;

		$data = $this->db->select("A.*, TO_CHAR(YOR_DATE,'DD/MM/YYYY') YOR_DATE2")->where('YOR_BRANCH_ID',$branch_id)->where("to_char(YOR_DATE,'MMYYYY')","'$date2'",false)->where('YOR_YARD_ID',$yard)->order_by("TO_CHAR(YOR_DATE,'YYYYMMDD')")->get('TH_REPORT_YOR A')->result_array();
	...
	public function reportYorExport(){
		...
		$user_id = (int)$this->session->isId;
		$yard = (int)$_GET['yard'];
		$date1 = $_GET['date'] != null? date('dmY',strtotime($_GET['date'])) : date('dmY');
		$date2 = $_GET['date'] != null? date('mY',strtotime($_GET['date'])) : date('mY');
		$date3 = $_GET['date'] != null? date('Y-m-d',strtotime($_GET['date'])) : date('Y-m-d');
		$date4 = $_GET['date'] != null? date('Y-m-d',strtotime(substr($_GET['date'],0,-1).'01')) : date('Y-m-d');
		//test
		// $yard = 203;
		// $date1 = '11082018';
		// $date2 = '082018';
		...
		$yesterday =  date('dmY',strtotime($date4.'- 1 days'));

		$data = $this->db->select("A.*, TO_CHAR(YOR_DATE,'DD/MM/YYYY') YOR_DATE2")->where('YOR_BRANCH_ID',$branch_id)->where("to_char(YOR_DATE,'MMYYYY')","'$date2'",false)->where('YOR_YARD_ID',$yard)->order_by("TO_CHAR(YOR_DATE,'YYYYMMDD')")->get('TH_REPORT_YOR A')->result_array();
		// $data2 = $this->db->where('YOR_BRANCH_ID',$branch_id)->where("YOR_DATE","to_date('$yesterday','DDMMYYYY')",false)->where('YOR_YARD_ID',$yard)->order_by("TO_CHAR(YOR_DATE,'YYYYMMDD')")->get('TH_REPORT_YOR')->row_array();
		// $yard_capacity = (int)$this->db->select_sum('BLOCK_CAPACITY')->where('BLOCK_YARD_ID',$yard)->where('BLOCK_ACTIVE','Y')->where('BLOCK_DUMMY','N')->where('BLOCK_BRANCH_ID',$branch_id)->get('TM_BLOCK')->row_array()['BLOCK_CAPACITY'];
		// $disable_cell = $this->db->where('CELL_YARD_ID',$yard)->where('CELL_BRANCH_ID',$branch_id)->from('TX_CELL_DISABLE')->count_all_results();
		// $ytp = $this->db->where('YBC_YARD_ID',$yard)->where('YBC_BRANCH_ID',$branch_id)->from('TX_YARD_BLOCK_CELL')->count_all_results();
		// $total_yor = $yard_capacity-$disable_cell;
		...
		// 	$arrData[$key] = $value;
		//
		// 	$date4 = date('d',strtotime($value['YOR_DATE']));
		// 	if($date4 == 01){
		// 		if($yesterday == '31072018'){
		// 			$box = 0;
		// 			$teus = 0;
		// 			$total_ytp = 0;
		// 		}
		// 		else{
		// 			$box = (int)$data2['YOR_REC_BOX'] - (int)$data2['YOR_DEL_BOX'];
		// 			$teus = (int)$data2['YOR_REC_TEUS'] - (int)$data2['YOR_DEL_TEUS'];
		// 			$total_ytp = (int)$data2['YOR_REC_TEUS'] / $ytp;
		// 		}
		//
		// 	}
		// 	else{
		// 		$box = (int)$value['YOR_REC_BOX'] - (int)$value['YOR_DEL_BOX'];
		// 		$teus = (int)$value['YOR_REC_TEUS'] - (int)$value['YOR_DEL_TEUS'];
		// 		$total_ytp = (int)$value['YOR_REC_TEUS'] / $ytp;
		// 	}
		//
		// 	$total_box = (int)$box+(int)$lastbox;
		// 	$total_teus = (int)$teus+(int)$lastteus;
		//
		// 	$arrData[$i]['BOX'] = $total_box;
		// 	$arrData[$i]['TEUS'] = $total_teus;
		// 	$arrData[$i]['YOR'] = round(($total_teus/$total_yor)*100,2);
		// 	$arrData[$i]['YTP'] = round($total_ytp,2);
		// 	$lastbox =  $total_box;
		// 	$lastteus =  $total_teus;
		// 	$i++;
		// }
		...
		$z = 0;
		$tot_rec_tpk_mty_20 = array();
		$tot_rec_tpk_mty_40 = array();
		$tot_rec_tpk_fl_20 = array();
		$tot_rec_tpk_fl_40 = array();
		$tot_rec_depo_mty_20 = array();
		$tot_rec_depo_mty_40 = array();
		$tot_rec_depo_fl_20 = array();
		$tot_rec_depo_fl_40 = array();
		$tot_box_rec = array();
		$tot_teus_rec = array();

		$tot_del_tpk_mty_20 = array();
		$tot_del_tpk_mty_40 = array();
		$tot_del_tpk_fl_20 = array();
		$tot_del_tpk_fl_40 = array();
		$tot_del_depo_mty_20 = array();
		$tot_del_depo_mty_40 = array();
		$tot_del_depo_fl_20 = array();
		$tot_del_depo_fl_40 = array();
		$tot_box_del = array();
		$tot_teus_del = array();
		...
		foreach ($data as $key => $value) {
			$arrData2[$key] = $value;
			//total
			$tot_rec_tpk_mty_20[$z] = $value['YOR_REC_TPK_MTY20'];
			$tot_rec_tpk_mty_40[$z] = $value['YOR_REC_TPK_MTY40'];
			$tot_rec_tpk_fl_20[$z] = $value['YOR_REC_TPK_FCL20'];
			$tot_rec_tpk_fl_40[$z] = $value['YOR_REC_TPK_FCL40'];

			$tot_rec_depo_mty_20[$z] = $value['YOR_REC_DEPO_MTY20'];
			$tot_rec_depo_mty_40[$z] = $value['YOR_REC_DEPO_MTY40'];
			$tot_rec_depo_fl_20[$z] = $value['YOR_REC_DEPO_FCL20'];
			$tot_rec_depo_fl_40[$z] = $value['YOR_REC_DEPO_FCL40'];

			$tot_box_rec[$z] = $value['YOR_REC_BOX'];
			$tot_teus_rec[$z] = $value['YOR_REC_TEUS'];

			$tot_del_tpk_mty_20[$z] = $value['YOR_DEL_TPK_MTY20'];
			$tot_del_tpk_mty_40[$z] = $value['YOR_DEL_TPK_MTY40'];
			$tot_del_tpk_fl_20[$z] = $value['YOR_DEL_TPK_FCL20'];
			$tot_del_tpk_fl_40[$z] = $value['YOR_DEL_TPK_FCL40'];
			$tot_del_depo_mty_20[$z] = $value['YOR_DEL_DEPO_MTY20'];
			$tot_del_depo_mty_40[$z] = $value['YOR_DEL_DEPO_MTY40'];
			$tot_del_depo_fl_20[$z] = $value['YOR_DEL_DEPO_FCL20'];
			$tot_del_depo_fl_40[$z] = $value['YOR_DEL_DEPO_FCL40'];
			$tot_box_del[$z] = $value['YOR_DEL_BOX'];
			$tot_teus_del[$z] = $value['YOR_DEL_TEUS'];
			$tot_box[$z] = $value['YOR_BOX'];
			$tot_teus[$z] = $value['YOR_TEUS'];
			$tot_yor[$z] = $value['YOR_TOTAL'];
			$tot_ytp[$z] = $value['YTP_TOTAL'];
			$z++;
		}

		$tot_rec_tpk_mty_20 = array_sum($tot_rec_tpk_mty_20);
		$tot_rec_tpk_mty_40 = array_sum($tot_rec_tpk_mty_40);
		$tot_rec_tpk_fl_20 = array_sum($tot_rec_tpk_fl_20);
		$tot_rec_tpk_fl_40 = array_sum($tot_rec_tpk_fl_40);

		$tot_rec_depo_mty_20 = array_sum($tot_rec_depo_mty_20);
		$tot_rec_depo_mty_40 = array_sum($tot_rec_depo_mty_40);
		$tot_rec_depo_fl_20 = array_sum($tot_rec_depo_fl_20);
		$tot_rec_depo_fl_40 = array_sum($tot_rec_depo_fl_40);

		$tot_box_rec = array_sum($tot_box_rec);
		$tot_teus_rec = array_sum($tot_teus_rec);

		$tot_del_tpk_mty_20 = array_sum($tot_del_tpk_mty_20);
		$tot_del_tpk_mty_40 = array_sum($tot_del_tpk_mty_40);
		$tot_del_tpk_fl_20 = array_sum($tot_del_tpk_fl_20);
		$tot_del_tpk_fl_40 = array_sum($tot_del_tpk_fl_40);
		$tot_del_depo_mty_20 = array_sum($tot_del_depo_mty_20);
		$tot_del_depo_mty_40 = array_sum($tot_del_depo_mty_40);
		$tot_del_depo_fl_20 = array_sum($tot_del_depo_fl_20);
		$tot_del_depo_fl_40 = array_sum($tot_del_depo_fl_40);
		$tot_box_del = array_sum($tot_box_del);
		$tot_teus_del = array_sum($tot_teus_del);
		$tot_box = array_sum($tot_box);
		$tot_teus = array_sum($tot_teus);
		$tot_yor = array_sum($tot_yor);
		$tot_ytp = array_sum($tot_ytp);

		$arrTotal = array();
		$arrTotal['tot_rec_tpk_mty_20'] = $tot_rec_tpk_mty_20;
		$arrTotal['tot_rec_tpk_mty_40'] = $tot_rec_tpk_mty_40;
		$arrTotal['tot_rec_tpk_fl_20'] = $tot_rec_tpk_fl_20;
		$arrTotal['tot_rec_tpk_fl_40'] =$tot_rec_tpk_fl_40;

		$arrTotal['tot_rec_depo_mty_20'] = $tot_rec_depo_mty_20;
		$arrTotal['tot_rec_depo_mty_40'] = $tot_rec_depo_mty_40;
		$arrTotal['tot_rec_depo_fl_20'] = $tot_rec_depo_fl_20;
		$arrTotal['tot_rec_depo_fl_40'] =$tot_rec_depo_fl_40;

		$arrTotal['tot_box_rec'] =$tot_box_rec;
		$arrTotal['tot_teus_rec'] =$tot_teus_rec;

		$arrTotal['tot_del_tpk_mty_20'] =$tot_del_tpk_mty_20;
		$arrTotal['tot_del_tpk_mty_40'] =$tot_del_tpk_mty_40;
		$arrTotal['tot_del_tpk_fl_20'] =$tot_del_tpk_fl_20;
		$arrTotal['tot_del_tpk_fl_40'] =$tot_del_tpk_fl_40;
		$arrTotal['tot_del_depo_mty_20'] =$tot_del_depo_mty_20;
		$arrTotal['tot_del_depo_mty_40'] =$tot_del_depo_mty_40;
		$arrTotal['tot_del_depo_fl_20'] =$tot_del_depo_fl_20;
		$arrTotal['tot_del_depo_fl_40'] =$tot_del_depo_fl_40;
		$arrTotal['tot_box_del'] =$tot_box_del;
		$arrTotal['tot_teus_del'] =$tot_teus_del;
		$arrTotal['tot_box'] =$tot_box;
		$arrTotal['tot_teus'] =$tot_teus;
		$arrTotal['tot_yor'] =$tot_yor;
		$arrTotal['tot_ytp'] =$tot_ytp;

		$arrAvg = array();
		$count = sizeof($arrData2);
		$arrAvg['avg_rec_tpk_mty_20'] = round($tot_rec_tpk_mty_20/$count,2);
		$arrAvg['avg_rec_tpk_mty_40'] = round($tot_rec_tpk_mty_40/$count,2);
		$arrAvg['avg_rec_tpk_fl_20'] = round($tot_rec_tpk_fl_20/$count,2);
		$arrAvg['avg_rec_tpk_fl_40'] = round($tot_rec_tpk_fl_40/$count,2);

		$arrAvg['avg_rec_depo_mty_20'] = round($tot_rec_depo_mty_20/$count,2);
		$arrAvg['avg_rec_depo_mty_40'] = round($tot_rec_depo_mty_40/$count,2);
		$arrAvg['avg_rec_depo_fl_20'] = round($tot_rec_depo_fl_20/$count,2);
		$arrAvg['avg_rec_depo_fl_40'] = round($tot_rec_depo_fl_40/$count,2);

		$arrAvg['avg_box_rec'] = round($tot_box_rec/$count,2);
		$arrAvg['avg_teus_rec'] = round($tot_teus_rec/$count,2);

		$arrAvg['avg_del_tpk_mty_20'] = round($tot_del_tpk_mty_20/$count,2);
		$arrAvg['avg_del_tpk_mty_40'] = round($tot_del_tpk_mty_40/$count,2);
		$arrAvg['avg_del_tpk_fl_20'] = round($tot_del_tpk_fl_20/$count,2);
		$arrAvg['avg_del_tpk_fl_40'] = round($tot_del_tpk_fl_40/$count,2);
		$arrAvg['avg_del_depo_mty_20'] = round($tot_del_depo_mty_20/$count,2);
		$arrAvg['avg_del_depo_mty_40'] = round($tot_del_depo_mty_40/$count,2);
		$arrAvg['avg_del_depo_fl_20'] = round($tot_del_depo_fl_20/$count,2);
		$arrAvg['avg_del_depo_fl_40'] = round($tot_del_depo_fl_40/$count,2);
		$arrAvg['avg_box_del'] = round($tot_box_del/$count,2);
		$arrAvg['avg_teus_del'] = round($tot_teus_del/$count,2);
		$arrAvg['avg_box'] = round($tot_box/$count,2);
		$arrAvg['avg_teus'] = round($tot_teus/$count,2);
		$arrAvg['avg_yor'] = round($tot_yor/$count,2);
		$arrAvg['avg_ytp'] = round($tot_ytp/$count,2);

		$data = array('DATA' => $arrData2, 'TOTAL' => $arrTotal, 'AVG' => $arrAvg);

		return $data;
	}

	function delete_yard_plan(){
		...
		$this->db->delete('TX_YARD_PLAN',  array('YP_CAT_HDR_ID' => $hdr_id));
		$this->db->delete('TX_YARD_PLAN_GROUP', array('YPG_CAT_HDR_ID' => $hdr_id));
		$this->db->delete('TX_CATEGORY_DTL', array('CAT_HDR_ID' => $hdr_id));
		$this->db->delete('TX_CATEGORY_HDR',  array('CAT_HDR_ID' => $hdr_id));
