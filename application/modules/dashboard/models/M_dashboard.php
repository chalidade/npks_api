...
class M_dashboard extends CI_Model {
	...
	public function getDataReceiving(){
		...
		$sql = "SELECT COUNT(DONE) DONE, COUNT(OUTSTANDING) OUTSTANDING, COUNT(DONE) + COUNT(OUTSTANDING) TOTAL FROM (SELECT * FROM (SELECT A.REQUEST_NO REQ, 'Receiving' ACTIVITY, B.REQUEST_DTL_CONT CONT, C.GATE_CREATE_DATE GATE_IN,
		(SELECT X.GATE_CREATE_DATE FROM TX_GATE X WHERE X.GATE_NOREQ = C.GATE_NOREQ AND X.GATE_CONT = C.GATE_CONT AND X.GATE_STATUS = 3 AND ROWNUM = 1) AS GATE_OUT, A.REQUEST_RECEIVING_DATE TGL_REQ,
		CASE WHEN C.GATE_CREATE_DATE IS NOT NULL AND (SELECT X.GATE_CREATE_DATE FROM TX_GATE X WHERE X.GATE_NOREQ = C.GATE_NOREQ AND X.GATE_CONT = C.GATE_CONT AND X.GATE_STATUS = 3 AND ROWNUM = 1) IS NOT NULL
		THEN 'DONE'
		ELSE
		NULL
		END DONE,
		...
		ELSE
		'OUTSTANDING'
		END OUTSTANDING, A.REQUEST_STATUS REQ_STATUS, A.REQUEST_BRANCH_ID BRANCH_ID, A.REQUEST_FROM ORIGIN
		FROM TX_REQ_RECEIVING_HDR A
		JOIN TX_REQ_RECEIVING_DTL B ON B.REQUEST_HDR_ID = A.REQUEST_ID
		LEFT JOIN TX_GATE C ON C.GATE_CONT = B.REQUEST_DTL_CONT AND C.GATE_STATUS = 1 AND C.GATE_NOREQ = A.REQUEST_NO) T
		WHERE T.BRANCH_ID = $branch_id AND REQ_STATUS != 2)";
	...
	public function getDataSp2(){
		...
		WHERE REQUEST_TO = 'DEPO' AND T.BRANCH_ID = $branch_id AND T.REQ_STATUS != 2)";
		$data = $this->db->query($sql)->result();
		return $data;
	}
	...
	public function getDataSp2Outstanding(){
		...
		$qWhere = "";
			...
			$query = "SELECT F.* FROM (SELECT T.*, ROWNUM r FROM (
				SELECT * FROM VIEW_DEL_DEPO_OUTSTANDING ORDER BY TGL_REQ DESC
			) T
	...
	public function getDataRepo(){
		...
		JOIN TX_REQ_DELIVERY_DTL B ON B.REQ_HDR_ID = A.REQ_ID
		LEFT JOIN TX_GATE C ON C.GATE_CONT = B.REQ_DTL_CONT AND C.GATE_STATUS = 1 AND C.GATE_NOREQ = A.REQ_NO) T
		WHERE REQUEST_TO = 'TPK' AND T.BRANCH_ID = $branch_id AND T.REQ_STATUS != 2)";
		$data = $this->db->query($sql)->result();
		return $data;
	}
	...
	public function getDataRepoOutstanding(){
		...
		$qWhere = "";
			...
			$query = "SELECT F.* FROM (SELECT T.*, ROWNUM r FROM (
				SELECT * FROM VIEW_DEL_REPO_OUTSTANDING ORDER BY TGL_REQ DESC
			) T
					WHERE BRANCH_ID = $branch_id $qWhere AND ROWNUM <= $end) F
						WHERE r >= $start + 1";
		$data = $this->db->query($query,$params)->result_array();
	...
	public function getDataStuffingOutstanding(){
		...
		$qWhere = "";
			...
			$query = "SELECT F.* FROM (SELECT T.*, ROWNUM r FROM (
				SELECT * FROM VIEW_STUFF_OUTSTANDING ORDER BY DATE_REQUEST DESC
			) T
					WHERE BRANCH_ID = $branch_id $qWhere AND ROWNUM <= $end) F
						WHERE r >= $start + 1";
		$data = $this->db->query($query,$params)->result_array();
	...
	public function getDataStrippingOutstanding(){
		...
		$qWhere = "";
			...
			$query = "SELECT F.* FROM (SELECT T.*, ROWNUM r FROM (
				SELECT * FROM VIEW_STRIP_OUTSTANDING ORDER BY DATE_REQUEST DESC
			) T
					WHERE BRANCH_ID = $branch_id $qWhere AND ROWNUM <= $end) F
						WHERE r >= $start + 1";
		$data = $this->db->query($query,$params)->result_array();
	...
	public function getDataYor(){
		...
		$stacking = $this->db->query("SELECT COUNT(A.REAL_YARD_ID) STACKING FROM TX_REAL_YARD A WHERE A.REAL_YARD_BRANCH_ID = $branch_id AND A.REAL_YARD_NO = $yard AND A.REAL_YARD_STATUS = 1 AND A.REAL_YARD_USED = 1")->row_array()['STACKING'];
