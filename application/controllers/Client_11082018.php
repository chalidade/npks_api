...
class Client extends CI_Controller {
									...
									if($totalcek <=0){
											...
											if($resultHDR){
													...
													while($a < $totalDTL){
															...
															$insertDTL = "INSERT INTO TX_REQ_DELIVERY_DTL (REQ_HDR_ID, REQ_DTL_CONT, REQ_DTL_CONT_STATUS, REQ_DTL_CONT_HAZARD, REQ_DTL_CONT_SIZE, REQ_DTL_CONT_TYPE, REQ_DTL_COMMODITY, REQ_DTL_STATUS)
																						VALUES (".$IDheader.", '".$REQ_DTL_CONT."','".$REQ_DTL_CONT_STATUS."','".$REQ_DTL_CONT_HAZARD."','".$REQ_DTL_SIZE."','".$REQ_DTL_TYPE."','".$REQ_DTL_COMMODITY."', '1')";
	...
	function getReceivingFromTPK(){
		...
		$sqlservices = "select SERVICES_ID, SERVICES_REQ_XML from TX_SERVICES where services_status = '0' AND services_method = 'getReceivingFromTPK' order by services_req_date desc";
		$resultservices = $this->db->query($sqlservices);
		$totalservice = $resultservices->num_rows();
		if($totalservice > 0){
			foreach($resultservices->result_array() as $row){
	...
	function getGateInFromTPK(){
		...
		$sqlservices = "select SERVICES_ID, SERVICES_REQ_XML from TX_SERVICES where services_status = '0' AND services_method = 'getGateInFromTPK' order by services_req_date desc";
		$resultservices = $this->db->query($sqlservices);
		$totalservice = $resultservices->num_rows();
		if($totalservice > 0){
			foreach($resultservices->result_array() as $row){
	...
	function getGateOutToTPK(){
		...
		$sqlservices = "select SERVICES_ID, SERVICES_REQ_XML from TX_SERVICES where services_status = '0' AND services_method = 'getGateOutToTPK' order by services_req_date desc";
		$resultservices = $this->db->query($sqlservices);
		$totalservice = $resultservices->num_rows();
		if($totalservice > 0){
			foreach($resultservices->result_array() as $row){
        	...
        	foreach($resultservices->result_array() as $row){
		        ...
		        if($valResponse != 0){		        	
		            ...
		            while ($i < $totalHDR) {
	                    ...
	                    $insertGATEIN = "INSERT INTO TX_GATE (GATE_NOREQ, GATE_CONT,GATE_CONT_SIZE, GATE_CONT_TYPE, GATE_CONT_STATUS, GATE_TRUCK_NO, GATE_MARK, GATE_ORIGIN, GATE_CREATE_DATE, GATE_STATUS, GATE_ACTIVITY, GATE_STACK_STATUS, GATE_FL_SEND, GATE_BRANCH_ID) VALUES 
	                    ...
	                    $resultHDR = $this->db->query($insertGATEIN);  
	                     ...
	                     $insertGATEOUT = "INSERT INTO TX_GATE (GATE_NOREQ, GATE_CONT,GATE_CONT_SIZE, GATE_CONT_TYPE,GATE_CONT_STATUS, GATE_TRUCK_NO, GATE_MARK, GATE_ORIGIN, GATE_CREATE_DATE, GATE_STATUS, GATE_ACTIVITY, GATE_STACK_STATUS, GATE_FL_SEND, GATE_BRANCH_ID) VALUES 
									...
									if($totalcek <=0){
											...
											if($resultHDR){					
													...
													while($a < $totalDTL){
															...
															$insertDTL = "INSERT INTO TX_REQ_STUFF_DTL (STUFF_DTL_HDR_ID, STUFF_DTL_CONT, STUFF_DTL_CONT_HAZARD, STUFF_DTL_CONT_SIZE, STUFF_DTL_CONT_TYPE, STUFF_DTL_COMMODITY, STUFF_DTL_REMARK_SP2, REQ_DTL_ORIGIN)
																						VALUES (".$IDheader.", '".$REQ_DTL_CONT."','".$REQ_DTL_CONT_HAZARD."','".$REQ_DTL_SIZE."','".$REQ_DTL_TYPE."','".$REQ_DTL_COMMODITY."','".$REQ_DTL_REMARK_SP2."','".$REQ_DTL_ORIGIN."')";
															$resultDtl = $this->db->query($insertDTL);
															if($resultDtl){
																	echo $a.", sukses detil | ".$REQ_DTL_CONT." ".date('Y-m-d H:i:s')."<br>\n";
															}else{
    ...
    function generateRealStripping(){
        ...
        if($totalservice > 0){
	        ...
	        $insert = $this->db->query($insertServices);
	        if($insert){
	        	$updateTxRealStat = "UPDATE TX_REAL_STRIP SET REAL_STRIP_FL_SEND = '1' WHERE REAL_STUFF_ID IN (".$IdData.") AND REAL_STRIP_FL_SEND = '0' ";
        		$this->db->query($updateTxRealStat);
	        	echo "Service ID ".$ServiceID." | ".date('Y-m-d H:i:s')."<br>\n";
	        }
        }else{
        	...
        	foreach($resultservices->result_array() as $row){
		        ...
		        if($valResponse != 0){
		            ...
		            while ($i < $totalData) {
		            	...
		            	$updateTxGate = "UPDATE TX_REAL_STRIP SET REAL_STRIP_FL_SEND = '9', REAL_STUFF_MARK = '".$KET."' WHERE REAL_STRIP_NOREQ = '".$NO_REQUEST."' AND REAL_STRIP_CONT = '".$NO_CONTAINER."' AND REAL_STRIP_FL_SEND = '1' ";
    					$this->db->query($updateTxGate);   
		                $i++;		            
		            }		            
	                $updateServices = "UPDATE TX_SERVICES SET SERVICES_STATUS = '9', SERVICES_DATE_GENERATE = SYSDATE WHERE SERVICES_ID = '".$row['SERVICES_ID']."' AND SERVICES_STATUS = '1' ";    	                
	                ...
	                if($totalcek <=0){
	                    ...
	                    if($resultHDR){
	                        ...
	                        while($a < $totalDTL){
	                            ...
	                            $insertDTL = "INSERT INTO TX_REQ_STRIP_DTL (STRIP_DTL_HDR_ID, STRIP_DTL_CONT, STRIP_DTL_DANGER, STRIP_DTL_CONT_SIZE, STRIP_DTL_CONT_TYPE, STRIP_DTL_COMMODITY, STRIP_DTL_STATUS)
	                                          VALUES (".$IDheader.", '".$REQ_DTL_CONT."','".$REQ_DTL_CONT_HAZARD."','".$REQ_DTL_SIZE."','".$REQ_DTL_TYPE."','".$REQ_DTL_COMMODITY."','1')";
	                            $resultDtl = $this->db->query($insertDTL);
	                            if($resultDtl){
	                                echo $a.", sukses detil | ".$REQ_DTL_CONT." ".date('Y-m-d H:i:s')."<br>\n";
	                            }else{
    ...
    function SetGateOut(){
        ...
        if($totalservice > 0){
        	foreach($resultservices->result_array() as $row){         		
		    	$client = "http://172.20.19.70/billing_npks_pnk/api.php";
			    $method = 'setGateOut'; //method
    ...
    function generatePlacement(){
    	$sqlgetIn = "SELECT A.REAL_YARD_ID, A.REAL_YARD_YBC_ID, B.YBC_SLOT, B.YBC_ROW, B.YBC_BLOCK_ID, A.REAL_YARD_TIER TIER, A.REAL_YARD_NO ID_YARD, A.REAL_YARD_CONT NO_CONTAINER, A.REAL_YARD_REQ_NO NO_REQUEST, A.REAL_YARD_BRANCH_ID, TO_CHAR(A.REAL_YARD_CREATE_DATE,'MM/DD/YYYY HH24:MI:SS') AS TGL_PLACEMENT, A.REAL_YARD_CREATE_BY AS ID_USER, A.REAL_YARD_CONT_STATUS CONT_STATUS
					...
					WHERE A.REAL_YARD_STATUS = '1' AND A.REAL_YARD_FL_SEND = '0' ORDER BY A.REAL_YARD_CREATE_DATE ASC";
    	$resultservices = $this->db->query($sqlgetIn);
        $totalservice = $resultservices->num_rows();
        if($totalservice > 0){
    		$returnXML = '<?xml version="1.0" encoding="UTF-8"?>';
