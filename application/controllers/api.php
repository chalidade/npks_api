...
{	
	if($user == 'npks' && $pass == '12345'){
		$db = getDB('storage');
		$query = "SELECT ID_REQ_SPPS NO_REQUEST, SUBSTR(ID_REQ_SPPS,0,3) JENIS, NO_CONTAINER, NO_BA, TO_CHAR(TANGGAL_PEMBUATAN,'MM/DD/YYYY HH24:MI:SS') TGL_REQUEST FROM REQ_BATAL_SPPS 	WHERE  TO_CHAR(TANGGAL_PEMBUATAN,'YYYYMMDDHH24MISS') > '".$tgl."' ORDER BY TO_CHAR(TANGGAL_PEMBUATAN,'YYYYMMDDHH24MISS') ASC";
...
{	
	if($user == 'npks' && $pass == '12345'){
        ...
        $query = "SELECT NO_CONTAINER, NO_REQUEST, STATUS_CONT, 'PELINDO' TRUK, 'TPK' DELIVERY_TO, KEGIATAN ,  TO_CHAR(TGL_UPDATE,'MM/DD/YYYY HH24:MI:SS') TGL_GATE 
				  ...
				  WHERE NO_REQUEST = '".$NoreqDelivery."' AND KEGIATAN = 'BORDER GATE OUT' ";
