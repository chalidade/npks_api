...
class M_gateContainer extends CI_Model {
  ...
  function getGateInConById(){
	...
	function ownerCheck(){
		...
		if($planningOwner <= 0 AND $activity == 2){
			$message =  false;
		}
  ...
  function setGateInContainer(){
				...
				foreach ($arrConfing as $key => $val) {
					...
					if(count($lokasi) > 0){
						...
						$lokasi = $this->db->query("SELECT B.CAT_HDR_ID, A.CAT_DTL_CONT_SIZE CONT, A.CAT_DTL_CONT_STATUS, A.CAT_DTL_CONT_TYPE, A.CAT_DTL_EXIM, C.YPG_YARD_ID, C.YPG_BLOCK_ID, C.YPG_STAR_ROW, C.YPG_END_ROW, C.YPG_START_SLOT, C.YPG_END_SLOT, C.YPG_CAPACITY, '' DI, D.BLOCK_NAME, E.YARD_NAME
											...
											JOIN TM_BLOCK D ON D.BLOCK_ID = C.YPG_BLOCK_ID AND D.BLOCK_ACTIVE = 'Y'
											JOIN TM_YARD E ON E.YARD_ID = C.YPG_YARD_ID
											WHERE B.CAT_BRANCH_ID = ".$branch_id." AND C.YPG_YARD_ID = ".$yard_active)->result_array();
						foreach ($lokasi as $loc) {
							$slot = $loc['YPG_START_SLOT'].' - '.$loc['YPG_END_SLOT'];
							$row = $loc['YPG_STAR_ROW'].' - '.$loc['YPG_END_ROW'];
							$arrGate[0]['GATE_TIME'] = $gate_time;
	...
	function getGateJobManager(){
		...
		if(!empty($_REQUEST['TRUCK_NO'])){
			...
			$query = "SELECT F.* FROM (SELECT T.*, ROWNUM r FROM (SELECT S.request_no, S.ACTIVITY, S.request_dtl_cont, S.request_consignee_id, S.consignee_name, S.gate_truck_no, S.gate_origin, S.ORIGIN_NAME,
	    NVL(TO_CHAR(S.GATE_IN,'DD/MM/YYYY HH24:MI'),'Belum Gate In') GATE_IN, NVL(TO_CHAR(S.GATE_OUT,'DD/MM/YYYY HH24:MI'),'Belum Gate Out') GATE_OUT,
	    TO_CHAR(TGL_REQ,'DD/MM/YYYY HH24:MI') TGL_REQ, NVL(TO_CHAR(S.PAID_THRU,'DD/MM/YYYY'),'-') PAID_THRU, CONT_SIZE, CONT_TYPE, CONT_STATUS
			FROM(SELECT * FROM(SELECT A.REQUEST_NO, 'Receiving' ACTIVITY, B.REQUEST_DTL_CONT, A.REQUEST_CONSIGNEE_ID, D.CONSIGNEE_NAME, C.GATE_TRUCK_NO, C.GATE_ORIGIN, E.REFF_NAME ORIGIN_NAME, C.GATE_CREATE_DATE GATE_IN,
			(SELECT X.GATE_CREATE_DATE FROM TX_GATE X WHERE X.GATE_NOREQ = C.GATE_NOREQ AND X.GATE_CONT = C.GATE_CONT AND X.GATE_STATUS = 3) AS GATE_OUT, A.REQUEST_RECEIVING_DATE TGL_REQ, NULL PAID_THRU,
			B.REQUEST_DTL_CONT_SIZE CONT_SIZE, B.REQUEST_DTL_CONT_TYPE CONT_TYPE, B.REQUEST_DTL_CONT_STATUS CONT_STATUS
			...
			UNION ALL
			SELECT * FROM(SELECT A.REQ_NO REQUEST_NO, 'Delivery' ACTIVITY, B.REQ_DTL_CONT REQUEST_DTL_CONT, A.REQ_CONSIGNEE_ID REQUEST_CONSIGNEE_ID, D.CONSIGNEE_NAME CONSIGNEE_NAME, C.GATE_TRUCK_NO GATE_TRUCK_NO, C.GATE_ORIGIN, E.REFF_NAME ORIGIN_NAME, C.GATE_CREATE_DATE GATE_IN,
			(SELECT X.GATE_CREATE_DATE FROM TX_GATE X WHERE X.GATE_NOREQ = C.GATE_NOREQ AND X.GATE_CONT = C.GATE_CONT AND X.GATE_STATUS = 3) AS GATE_OUT, A.REQ_DELIVERY_DATE TGL_REQ, B.REQ_DTL_DEL_DATE PAID_THRU,
						B.REQ_DTL_CONT_SIZE CONT_SIZE, B.REQ_DTL_CONT_TYPE CONT_TYPE, B.REQ_DTL_CONT_STATUS CONT_STATUS
		...
		$count = $this->db->query("SELECT S.request_no, S.ACTIVITY, S.request_dtl_cont, S.request_consignee_id, S.consignee_name, S.gate_truck_no, S.gate_origin, S.ORIGIN_NAME,
		NVL(TO_CHAR(S.GATE_IN,'DD/MM/YYYY HH24:MI'),'Belum Gate In') GATE_IN, NVL(TO_CHAR(S.GATE_OUT,'DD/MM/YYYY HH24:MI'),'Belum Gate Out') GATE_OUT,
		TO_CHAR(TGL_REQ,'DD/MM/YYYY HH24:MI') TGL_REQ, NVL(TO_CHAR(S.PAID_THRU,'DD/MM/YYYY'),'-') PAID_THRU, CONT_SIZE, CONT_TYPE, CONT_STATUS
		FROM(SELECT * FROM(SELECT A.REQUEST_NO, 'Receiving' ACTIVITY, B.REQUEST_DTL_CONT, A.REQUEST_CONSIGNEE_ID, D.CONSIGNEE_NAME, C.GATE_TRUCK_NO, C.GATE_ORIGIN, E.REFF_NAME ORIGIN_NAME, C.GATE_CREATE_DATE GATE_IN,
		(SELECT X.GATE_CREATE_DATE FROM TX_GATE X WHERE X.GATE_NOREQ = C.GATE_NOREQ AND X.GATE_CONT = C.GATE_CONT AND X.GATE_STATUS = 3) AS GATE_OUT, A.REQUEST_RECEIVING_DATE TGL_REQ, NULL PAID_THRU,
	...
	function getGateReport(){
		...
		//apply filter
			...
			CASE WHEN (ROUND(SYSDATE - GATE_IN,2)) < 1 THEN 1 ELSE ROUND(SYSDATE - GATE_IN,2) END
			ELSE
			CASE WHEN GATE_IN IS NOT NULL AND GATE_OUT IS NOT NULL THEN
			CASE WHEN (ROUND(GATE_OUT - GATE_IN,2)) < 1 THEN 1 ELSE ROUND(GATE_OUT - GATE_IN,2) END
			ELSE
			NULL
			END
			END TRT
			...
			$cont_rec = $this->db->select("A.REQUEST_DTL_CONT, A.REQUEST_DTL_CONT_SIZE, A.REQUEST_DTL_CONT_TYPE, B.REQUEST_DI DI, A.REQUEST_DTL_CONT_STATUS, A.REQUEST_DTL_OWNER_CODE, A.REQUEST_DTL_OWNER_NAME")
				...
				//crate array for print
					...
					$lokasi = $this->db->query("SELECT A.CMS_NOREQ, A.CMS_CONT, A.CMS_BRANCH_ID, A.CMS_YARD_ID, A.CMS_BLOCK_ID, A.CMS_SLOT_ID, A.CMS_TRUCK, B.YARD_NAME, C.BLOCK_NAME FROM TX_CMS A
						 				JOIN TM_YARD B ON A.CMS_YARD_ID = B.YARD_ID
										JOIN TM_BLOCK C ON A.CMS_BLOCK_ID = C.BLOCK_ID
