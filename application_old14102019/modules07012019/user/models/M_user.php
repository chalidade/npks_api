...
class M_user extends CI_Model {
	...
	public function getUsers($filter){
		...
		if(!in_array($user_role,$admin)){
			$whereParams .= ' AND USER_ID = '.$filter['USER_ID'];
		}
		...
		$sql = "
				...
				(
					SELECT TABLE_1.*, rownum AS rnum FROM (
							SELECT * FROM USER_VIEW WHERE 1=1 $whereParams ORDER BY USER_ID
					) TABLE_1
					WHERE ROWNUM <= ?
				)
				WHERE  rnum >= ? + 1";
		...
		$sqlTotal = "SELECT USER_ID FROM USER_VIEW WHERE 1=1 " . $whereParams;
