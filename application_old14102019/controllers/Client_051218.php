...
class Client extends CI_Controller {
	...
	function getServer(){
		...
		$ch = curl_init(SERVICE_SERVER_NODEJS."/updateGateJobManager?branch=3");
		curl_exec($ch);
		curl_close($ch);
		//echo SERVICE_SERVER;
	}
        	...
        	foreach($resultservices->result_array() as $row){
		        ...
		        if($valResponse != 0){
		            ...
		            while ($i < $totalData) {
		            	...
		            	$insert = "INSERT INTO TX_REQ_DELIVERY_DTL (CONT_NO,CONT_TYPE,CONT_SIZE,CONT_STATUS,BLOCK,SLOT,\"ROW\",TIER,NO_REQ,STACK_DATE)
												VALUES ('".$CONT_NO."','".$CONT_TYPE."','".$CONT_SIZE."','".$CONT_STATUS."','".$BLOCK."','".$SLOT."', '".$ROW."', '".$TIER."', '".$NO_REQ."',TO_DATE('".$STACK_DATE."','MM/DD/YYYY HH24:MI:SS'))";
						$result = $this->db->query($insert); 
						if($result){
							echo $a.", sukses insert | ".$CONT_NO." ".date('Y-m-d H:i:s')."<br>\n";
						}else{
							echo $a.", gagal insert | ".$CONT_NO." ".date('Y-m-d H:i:s')."<br>\n";
						}
							...
							while ($i < $totalHDR) {
									...
									if($totalcek <=0){
											...
											if($resultHDR){
													...
													//start call nodejs
														$updateGateJobManager = curl_init(SERVICE_SERVER_NODEJS."/updateGateJobManager?branch=3");													
														curl_exec($updateGateJobManager);
														curl_close($updateGateJobManager);
														$updateSP2 = curl_init(SERVICE_SERVER_NODEJS."/updateSP2?branch=3");
														curl_exec($updateSP2);
		                ...
		                if($totalcek <=0){
		                    ...
		                    if($resultHDR){
		                        ...
		                        while($a < $totalDTL){
									...
									$sqlcekmstcont = "SELECT CONTAINER_NO FROM TM_CONTAINER WHERE CONTAINER_NO='".$REQ_DTL_CONT."' AND CONTAINER_BRANCH_ID = 3 ";
				                    $resultCekmstcont = $this->db->query($sqlcekmstcont);
				                    $totalcekmstcont = $resultCekmstcont->num_rows();
				                    if($totalcekmstcont >0){
				                        $updatecontowner = "UPDATE TM_CONTAINER SET CONTAINER_OWNER = '".$REQUEST_DTL_OWNER_CODE."' WHERE CONTAINER_NO='".$REQ_DTL_CONT."' AND CONTAINER_BRANCH_ID = 3 ";
		                        ...
		                        //start call nodejs
									...
									curl_exec($updateGateJobManager);
									curl_close($updateGateJobManager);
									$updateReceiving = curl_init(SERVICE_SERVER_NODEJS."/updateReceiving?branch=3");
        	...
        	foreach($resultservices->result_array() as $row){
		        ...
		        if($valResponse != 0){		        	
		            ...
		            while ($i < $totalHDR) {
        				...
        				//get cont size, type
        				$sql_getCont = "SELECT  REAL_YARD_CONT_STATUS, REAL_YARD_CONT_SIZE, REAL_YARD_CONT_TYPE FROM TX_REAL_YARD WHERE REAL_YARD_ID = (SELECT MAX(REAL_YARD_ID) FROM TX_REAL_YARD WHERE REAL_YARD_CONT = '".$NO_CONT."' AND REAL_YARD_BRANCH_ID = 3)";
							...
							while ($i < $totalHDR) {
									...
									if($totalcek <=0){
											...
											if($resultHDR){					
													...
													//start call nodejs
														$updateStuffingProcess = curl_init(SERVICE_SERVER_NODEJS."/updateStuffingProcess?branch=3");													
														curl_exec($updateStuffingProcess);
														curl_close($updateStuffingProcess);

														$updateGateJobTruckManager = curl_init(SERVICE_SERVER_NODEJS."/updateGateJobTruckManager?branch=3");													
    ...
    function generateRealStripping(){
		$sqlgetStrip = "SELECT A.REAL_STRIP_ID, A.REAL_STRIP_CONT NO_CONT, A.REAL_STRIP_NOREQ NO_REQUEST, TO_CHAR(B.STRIP_CREATE_DATE,'MM/DD/YYYY HH24:MI:SS') AS TGL_REQUEST,  A.REAL_STRIP_MECHANIC_TOOLS ALAT, TO_CHAR(A.REAL_STRIP_DATE,'MM/DD/YYYY HH24:MI:SS') AS TGL_REALISASI,A.REAL_STRIP_BY ID_USER, C.REAL_YARD_YBC_ID YBC_ID, A.REAL_STRIP_MARK  
					...
					WHERE A.REAL_STRIP_FL_SEND = 0 AND A.REAL_STRIP_STATUS = 2 AND A.REAl_STRIP_BRANCH_ID = 3
	            ...
	            while ($i < $totalHDR) {
	                ...
	                if($totalcek <=0){
	                    ...
	                    if($resultHDR){
	                        ...
	                        //start call nodejs
								$updateStrippingProcess = curl_init(SERVICE_SERVER_NODEJS."/updateStrippingProcess?branch=3");													
    ...
    function getBatalMuat(){
        ...
        $sqlLastDateNota = "select TGL from (
                            select TO_CHAR(CANCELLED_CREATE_DATE,'YYYYMMDDHH24MISS') TGL from  TH_CANCELLED  WHERE CANCELED_STATUS IN (1,2,3,4,5,6) AND CANCELLED_BRANCH_ID = 3  order by CANCELLED_CREATE_DATE DESC
                            )A where rownum =1";
		            ...
		            while ($i < $totalData) {
						...
						while($a < $totalDTL){
							...
							$insertCancelled = "INSERT INTO TH_CANCELLED (CANCELLED_NOREQ, CANCELLED_NO_CONT, CANCELLED_STATUS, CANCELLED_NOREQ_OLD, CANCELLED_REQ_DATE, CANCELLED_BRANCH_ID)
	        									VALUES ('".$NO_REQUEST."','".$NO_CONTAINER."','".$stat."','".$NO_REQ_BATAL."', TO_DATE('".$TGLREQUEST."','MM/DD/YYYY HH24:MI:SS'), 3)";
	       					$this->db->query($insertCancelled);

							if($STATUS_GATE == '1' || $STATUS_GATE == '3'){
								$sqlDtl = "SELECT A.STUFF_DTL_ID, B.STUFF_ID FROM TX_REQ_STUFF_DTL A INNER JOIN TX_REQ_STUFF_HDR B ON B.STUFF_ID = A.STUFF_DTL_HDR_ID WHERE A.STUFF_DTL_CONT = '".$NO_CONTAINER."' AND B.STUFF_NO = '".$NO_REQ_BATAL."'";
								...
								$sqlDtl = "SELECT A.REQ_DTL_ID, B.REQ_ID FROM TX_REQ_DELIVERY_DTL A INNER JOIN TX_REQ_DELIVERY_HDR B ON B.REQ_ID = A.REQ_HDR_ID WHERE A.REQ_DTL_CONT = '".$NO_CONTAINER."' AND B.REQ_NO = '".$NO_REQ_BATAL."'";
						        $resultID = $this->db->query($sqlDtl)->row_array();
						...
						$SelectMaterCont = "SELECT CONTAINER_TYPE, CONTAINER_SIZE FROM TM_CONTAINER WHERE CONTAINER_NO = '".$NO_CONTAINER."' ";
    ...
    function getBatalStuffStrip(){
		$client = SERVICE_SERVER."/billing_npks_pnk/api.php";//alamat web service     http://172.20.19.70/billing_npks_pnk/login
        $method = 'getBatalStuffStrip'; //method

        $sqlLastDateNota = "select TGL from (
                            select TO_CHAR(CANCELLED_CREATE_DATE,'YYYYMMDDHH24MISS') TGL from  TH_CANCELLED  WHERE CANCELED_STATUS IN (7,8) AND CANCELLED_BRANCH_ID = 3  order by CANCELLED_CREATE_DATE DESC
                            )A where rownum =1";
	...
	function ParsingBatalStuffStrip(){
		        ...
		        if($valResponse != 0){		        	
		            ...
		            while ($i < $totalData) {
						...
						$insertCancelled = "INSERT INTO TH_CANCELLED (CANCELLED_NOREQ, CANCELLED_NO_CONT, CANCELLED_STATUS, CANCELLED_NOREQ_OLD, CANCELLED_REQ_DATE, CANCELLED_MARK, CANCELLED_BRANCH_ID)
        									VALUES ('".$REQ_NO."','".$NO_CONTAINER."','".$stat."','".$REQ_NO."', TO_DATE('".$TGL_REQUEST."','MM/DD/YYYY HH24:MI:SS'), '".$NO_BA."', 3)";
       					$this->db->query($insertCancelled);
		                ...
		                if($JENIS == 'STR'){
		                	...
		                	$sqlDtl = "SELECT A.STUFF_DTL_ID, B.STUFF_ID, B.STUFF_NOREQ_RECEIVING FROM TX_REQ_STUFF_DTL A INNER JOIN TX_REQ_STUFF_HDR B ON B.STUFF_ID = A.STUFF_DTL_HDR_ID WHERE A.STUFF_DTL_CONT = '".$NO_CONTAINER."' AND B.STUFF_NO = '".$REQ_NO."'";
					        $resultID = $this->db->query($sqlDtl)->row_array();
					        $DTL_ID = $resultID['REQ_ID'];
					        $HDR_ID = $resultID['STUFF_ID'];
		                ...
		                $sqlDtl = "SELECT A.REQ_DTL_ID, B.REQ_ID FROM TX_REQ_DELIVERY_DTL A INNER JOIN TX_REQ_DELIVERY_HDR B ON B.REQ_ID = A.REQ_HDR_ID WHERE A.REQ_DTL_CONT = '".$NO_CONTAINER."' AND B.REQ_NO = '".$NO_REC."'";
				        $resultID = $this->db->query($sqlDtl)->row_array();
				        $DTL_ID = $resultID['REQ_DTL_ID'];
				        $HDR_ID = $resultID['REQ_ID'];
    ...
    function getBatalOperation(){
		$client = SERVICE_SERVER."/billing_npks_pnk/api.php";//alamat web service     http://172.20.19.70/billing_npks_pnk/login
        $method = 'getBatalOperation'; //method

        $sqlLastDateNota = "select TGL from (
                            select TO_CHAR(CANCELLED_CREATE_DATE,'YYYYMMDDHH24MISS') TGL from  TH_CANCELLED  WHERE CANCELED_STATUS IN (9,19,11,12,13,14,15) AND CANCELLED_BRANCH_ID = 3  order by CANCELLED_CREATE_DATE DESC
                            )A where rownum =1";
	...
	function ParsingBatalOperation(){
        	...
        	foreach($resultservices->result_array() as $row){
		        ...
		        if($valResponse != 0){		        	
		            ...
		            while ($i < $totalData) {
						...
						$insertCancelled = "INSERT INTO TH_CANCELLED (CANCELLED_NOREQ, CANCELLED_NO_CONT, CANCELLED_STATUS, CANCELLED_NOREQ_OLD, CANCELLED_REQ_DATE, CANCELLED_MARK, CANCELLED_BRANCH_ID)
        									VALUES ('".$NO_REQUEST."','".$NO_CONTAINER."','".$stat."','".$NO_REQUEST."', TO_DATE('".$TGL_REQUEST."','MM/DD/YYYY HH24:MI:SS'), '".$KEGIATAN."',3)";
       					$this->db->query($insertCancelled);
		                
		                if($KEGIATAN == 'REALISASI STRIPPING'){
		                	$sqlDtl = "SELECT A.STRIP_DTL_ID, B.STRIP_ID, B.STRIP_NOREQ_RECEIVING FROM TX_REQ_STRIP_DTL A INNER JOIN TX_REQ_STRIP_HDR B ON B.STRIP_ID = A.STRIP_DTL_HDR_ID WHERE A.STRIP_DTL_CONT = '".$NO_CONTAINER."' AND B.STRIP_NO = '".$NO_REQUEST."'";
					        $resultID = $this->db->query($sqlDtl)->row_array();
					        $DTL_ID = $resultID['STRIP_DTL_ID'];
					        $HDR_ID = $resultID['STRIP_ID'];
					        $NO_REC = $resultID['STRIP_NOREQ_RECEIVING'];
		                	...
		                	$sqlDtl = "SELECT A.STUFF_DTL_ID, B.STUFF_ID, B.STUFF_NOREQ_RECEIVING FROM TX_REQ_STUFF_DTL A INNER JOIN TX_REQ_STUFF_HDR B ON B.STUFF_ID = A.STUFF_DTL_HDR_ID WHERE A.STUFF_DTL_CONT = '".$NO_CONTAINER."' AND B.STUFF_NO = '".$NO_REQUEST."'";
					        $resultID = $this->db->query($sqlDtl)->row_array();
					        $DTL_ID = $resultID['REQ_ID'];
					        $HDR_ID = $resultID['STUFF_ID'];
					        $NO_REC = $resultID['STUFF_NOREQ_RECEIVING'];
			                ...
			                $sqlDtl = "SELECT A.REQ_DTL_ID, B.REQ_ID FROM TX_REQ_DELIVERY_DTL A INNER JOIN TX_REQ_DELIVERY_HDR B ON B.REQ_ID = A.REQ_HDR_ID WHERE A.REQ_DTL_CONT = '".$NO_CONTAINER."' AND B.REQ_NO = '".$NO_REQUEST."'";
					        $resultID = $this->db->query($sqlDtl)->row_array();
					        $DTL_ID = $resultID['REQ_DTL_ID'];
					        $HDR_ID = $resultID['REQ_ID'];
							...
							$sqlDtl = "SELECT A.REQUEST_DTL_ID, B.REQUEST_ID FROM TX_REQ_RECEIVING_DTL A INNER JOIN TX_REQ_RECEIVING_HDR B ON B.REQUEST_ID = A.REQUEST_HDR_ID WHERE A.REQUEST_DTL_CONT = '".$NO_CONTAINER."' AND B.REQUEST_NO = '".$NO_REQUEST."' AND B.REQUEST_BRANCH_ID = 3";
					        $resultID = $this->db->query($sqlDtl)->row_array();
	...
	function get_all_api(){
		...
		foreach ($allData as $data) {
			echo base_url().'client/'.$data->API_CLIENT_NAME.'/'.$branch;
			echo "<br/>";
			$ch = curl_init(base_url().'client/'.$data->API_CLIENT_NAME.'/'.$branch);
			curl_exec($ch);
			curl_close($ch);
			echo "<br/>";
		}
