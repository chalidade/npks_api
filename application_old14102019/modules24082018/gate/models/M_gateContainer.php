  ...
  function setGateInContainer(){
	...
	function getGateReport(){
		...
		//apply filter
			...
			$query = "SELECT F.* FROM (SELECT T.*, ROWNUM r FROM (SELECT REQUEST_NO, ACTIVITY_CODE, ACTIVITY, REQUEST_DTL_CONT, REQUEST_CONSIGNEE_ID, CONSIGNEE_NAME, GATE_TRUCK_NO, GATE_ORIGIN, ORIGIN_NAME, TO_CHAR(GATE_IN,'DD/MM/YYYY HH24:MI') GATE_IN, TO_CHAR(GATE_OUT,'DD/MM/YYYY HH24:MI') GATE_OUT FROM(SELECT * FROM(SELECT A.REQUEST_NO, 3 ACTIVITY_CODE, 'Receiving' ACTIVITY, B.REQUEST_DTL_CONT, A.REQUEST_CONSIGNEE_ID, D.CONSIGNEE_NAME, C.GATE_TRUCK_NO, C.GATE_ORIGIN, E.REFF_NAME ORIGIN_NAME, C.GATE_CREATE_DATE GATE_IN,
			...
			ORDER BY A.REQUEST_ID DESC)
			UNION ALL
			SELECT * FROM(SELECT A.REQ_NO REQUEST_NO, 4 ACTIVITY_CODE, 'Delivery' ACTIVITY, B.REQ_DTL_CONT REQUEST_DTL_CONT, A.REQ_CONSIGNEE_ID REQUEST_CONSIGNEE_ID, D.CONSIGNEE_NAME CONSIGNEE_NAME, C.GATE_TRUCK_NO GATE_TRUCK_NO, C.GATE_ORIGIN, E.REFF_NAME ORIGIN_NAME, C.GATE_CREATE_DATE GATE_IN,
			(SELECT X.GATE_CREATE_DATE FROM TX_GATE X WHERE X.GATE_NOREQ = C.GATE_NOREQ AND X.GATE_CONT = C.GATE_CONT AND X.GATE_STATUS = 3) AS GATE_OUT
						FROM TX_REQ_DELIVERY_HDR A
						JOIN TX_REQ_DELIVERY_DTL B ON B.REQ_HDR_ID = A.REQ_ID
						LEFT JOIN TX_GATE C ON C.GATE_CONT = B.REQ_DTL_CONT AND C.GATE_STATUS = 1 AND C.GATE_NOREQ = A.REQ_NO
