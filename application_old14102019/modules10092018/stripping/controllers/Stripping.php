...
class Stripping extends CI_Controller {
  ...
  public function getContainer(){
    $data = $this->m_stripping->getContainer();
  ...
  public function getContainerById(){
    if($id = $this->check_token()){
      if($id == $this->session->userdata('isId')){
        $return = $this->m_stripping->getContainerById();
      }
      else{
        $return = array(
          'success' => false,
  ...
  public function cekStrippingRun(){
    $return = $this->m_stripping->cekStrippingRun();
    header('Content-Type: text/javascript');
    die(json_encode($return));
  }

  public function insertStripping(){
    if($id = $this->check_token()){
      if($id == $this->session->userdata('isId')){
        $data = $this->m_stripping->setStripping();
        $return = array(
          'success' => true,
          'message' => $data
        );
