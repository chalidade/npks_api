  ...
  function setGateInContainer(){
	...
	function getGateJobManager(){
		...
		if(!empty($_REQUEST['TRUCK_NO'])){
			...
			NVL((SELECT to_char(X.GATE_CREATE_DATE,'DD/MM/YYYY HH24:MI') FROM TX_GATE X WHERE X.GATE_NOREQ = C.GATE_NOREQ AND X.GATE_CONT = C.GATE_CONT AND X.GATE_STATUS = 3),'Belum Gate Out') AS GATE_OUT, TO_CHAR(A.REQUEST_RECEIVING_DATE,'DD/MM/YYYY HH24:MI') TGL_REQ, '-' PAID_THRU,
			...
			NVL((SELECT to_char(X.GATE_CREATE_DATE,'DD/MM/YYYY HH24:MI') FROM TX_GATE X WHERE X.GATE_NOREQ = C.GATE_NOREQ AND X.GATE_CONT = C.GATE_CONT AND X.GATE_STATUS = 3),'Belum Gate Out') AS GATE_OUT, TO_CHAR(A.REQ_DELIVERY_DATE,'DD/MM/YYYY HH24:MI') TGL_REQ, TO_CHAR(B.REQ_DTL_DEL_DATE,'DD/MM/YYYY') PAID_THRU,
