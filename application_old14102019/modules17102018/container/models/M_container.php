	...
	public function getStuffingHistory($filter){
		...
		$sql = "SELECT * FROM
				(
					...
					WHERE A.STUFF_STATUS <> '2' AND B.STUFF_DTL_ACTIVE = 'Y' AND A.STUFF_BRANCH_ID = ?
					) TABLE_1
					WHERE ROWNUM <= ?
				)
				WHERE  rnum >= ? + 1" .$qWhere;
    	...
    	$sqlTotal = "SELECT * FROM(SELECT B.STUFF_DTL_CONT CONTAINER_NUMBER, A.STUFF_NO REQUEST_NUMBER,
					...
					INNER JOIN TX_REQ_STUFF_DTL B ON B.STUFF_DTL_HDR_ID = A.STUFF_ID
                    JOIN TM_CONSIGNEE C ON C.CONSIGNEE_ID = A.STUFF_CONSIGNEE_ID
					WHERE A.STUFF_STATUS <> '2' AND B.STUFF_DTL_ACTIVE = 'Y' AND A.STUFF_BRANCH_ID = ?) WHERE 1=1 " . $qWhere;
	...
	public function getStrippingHistory($filter){
		...
		$sql = "SELECT * FROM (
          ...
          C.CONSIGNEE_NAME EMKL
					...
					WHERE A.STRIP_STATUS <> 2 AND B.STRIP_DTL_ACTIVE = 'Y' AND A.STRIP_BRANCH_ID = ?
					) TABLE_1
					WHERE ROWNUM <= ?
				)
				WHERE  rnum >= ? + 1" .$qWhere;
		...
		TX_REQ_STRIP_HDR A
		INNER JOIN TX_REQ_STRIP_DTL B ON B.STRIP_DTL_HDR_ID = A.STRIP_ID
		WHERE A.STRIP_STATUS <> 2 AND B.STRIP_DTL_ACTIVE = 'Y' AND A.STRIP_BRANCH_ID = ?) WHERE 1=1 " . $qWhere;
