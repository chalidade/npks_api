...
$query 		= "SELECT T1.*, (SELECT MAX(G.CAT_DTL_CONT_SIZE) FROM TX_YARD_PLAN_GROUP E
				...
				FROM (
					SELECT A.YBC_YARD_ID ,A.YBC_CELL_ID, B.BLOCK_ID, B.BLOCK_NAME, B.BLOCK_POSITION, B.BLOCK_ORIENTATION, B.BLOCK_TIER, A.YBC_ROW, A.YBC_SLOT, B.BLOCK_ROW MAX_ROW, B.BLOCK_SLOT MAX_SLOT, A.YBC_BRANCH_ID,
						MAX(C.YP_STATUS) FLAG_STATUS,SUM(DECODE(NVL(C.YP_STATUS,0),0,0,1)) JML_TAKEN, 0 PLACEMENT, NVL(FUNC_GET_TOT_STACK(A.YBC_ID, A.YBC_BRANCH_ID),0) JML_PLACEMENT, B.BLOCK_ROW, B.BLOCK_SLOT, D.YARD_HEIGHT, D.YARD_WIDTH, A.YBC_ID
						FROM TX_YARD_BLOCK_CELL A
						JOIN TM_BLOCK B ON A.YBC_BLOCK_ID = B.BLOCK_ID
						LEFT JOIN TX_YARD_PLAN C ON A.YBC_ID = C.YP_YBC_ID
						JOIN TM_YARD D ON B.BLOCK_YARD_ID = D.YARD_ID
						WHERE A.YBC_ACTIVE = 'Y' AND B.BLOCK_ACTIVE = 'Y' AND A.YBC_YARD_ID = ? AND A.YBC_BRANCH_ID = ?
						GROUP BY A.YBC_YARD_ID ,A.YBC_CELL_ID, B.BLOCK_ID, B.BLOCK_NAME, B.BLOCK_POSITION, B.BLOCK_ORIENTATION, B.BLOCK_TIER, A.YBC_ROW, A.YBC_SLOT, B.BLOCK_ROW, B.BLOCK_SLOT, A.YBC_BRANCH_ID, D.YARD_HEIGHT, D.YARD_WIDTH, A.YBC_ID
...
$xml_str = "<yard>".$width_str.$height_str.$stack_str.$plan_str.$taken_str.$placement_str.$slot_str.$max_slot_str.$row_str.$max_row_str.$tier_str.$title_str.$block_id_str.$orientation_str.$position_str.$label_str.$label_text_str.$ybc_id_str.$cont_size_str."</yard>";
...
public function get_yard_stacking($id_yard, $block, $slot, $ybc_id){
  ...
  return $query2;
	...
	public function reportYor(){
		...
		$yard_capacity = (int)$this->db->select_sum('BLOCK_CAPACITY')->where('BLOCK_YARD_ID',$yard)->where('BLOCK_BRANCH_ID',$branch_id)->get('TM_BLOCK')->row_array()['BLOCK_CAPACITY'];
