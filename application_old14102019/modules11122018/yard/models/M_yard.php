...
$query 		= "SELECT T1.*, (SELECT MAX(G.CAT_DTL_CONT_SIZE) FROM TX_YARD_PLAN_GROUP E
				...
				FROM (
					SELECT A.YBC_YARD_ID ,A.YBC_CELL_ID, B.BLOCK_ID, B.BLOCK_NAME, B.BLOCK_POSITION, B.BLOCK_ORIENTATION, B.BLOCK_TIER, A.YBC_ROW, A.YBC_SLOT, B.BLOCK_ROW MAX_ROW, B.BLOCK_SLOT MAX_SLOT, A.YBC_BRANCH_ID,
						MAX(C.YP_STATUS) FLAG_STATUS,SUM(DECODE(NVL(C.YP_STATUS,0),0,0,1)) JML_TAKEN, 0 PLACEMENT, NVL(FUNC_GET_TOT_STACK(A.YBC_ID, A.YBC_BRANCH_ID),0) JML_PLACEMENT, B.BLOCK_ROW, B.BLOCK_SLOT, D.YARD_HEIGHT, D.YARD_WIDTH, A.YBC_ID, B.BLOCK_DUMMY
						FROM TX_YARD_BLOCK_CELL A
						JOIN TM_BLOCK B ON A.YBC_BLOCK_ID = B.BLOCK_ID
						LEFT JOIN TX_YARD_PLAN C ON A.YBC_ID = C.YP_YBC_ID
						JOIN TM_YARD D ON B.BLOCK_YARD_ID = D.YARD_ID
...
$xml_str = "<yard>".$width_str.$height_str.$stack_str.$plan_str.$taken_str.$placement_str.$slot_str.$max_slot_str.$row_str.$max_row_str.$tier_str.$title_str.$block_id_str.$orientation_str.$position_str.$label_str.$label_text_str.$ybc_id_str.$cont_size_str.$label_block_dummy."</yard>";
...
public function get_yard_stacking($id_yard, $block, $slot, $ybc_id){
  ...
  return $query2;
	...
	public function reportYor(){
		...
		$yesterday2 =  date('dmY',strtotime($date3.'- 1 days'));
		...
		$hariSebelumnya = $this->db->where('YOR_BRANCH_ID',$branch_id)->where("YOR_DATE","to_date('$yesterday2','DDMMYYYY')",false)->where('YOR_YARD_ID',$yard)->order_by("TO_CHAR(YOR_DATE,'YYYYMMDD')")->get('TH_REPORT_YOR')->row_array();
		...
		foreach ($data as $key => $value) {
			...
			if($date4 == 01){
				if($yesterday == '30092018'){
					$box = 2894;
					$teus = 3276;
					$total_ytp = 0;
				}
				else{
					$box = (int)$data2['YOR_BOX'] + (int)$data2['YOR_REC_BOX'] - (int)$data2['YOR_DEL_BOX'];
					$teus = (int)$data2['YOR_TEUS'] + (int)$data2['YOR_REC_TEUS'] - (int)$data2['YOR_DEL_TEUS'];
					$total_ytp = (int)$data2['YOR_REC_TEUS'] / $ytp;
				}

			}
			else{
				$box = (int)$hariSebelumnya['YOR_BOX'] + (int)$value['YOR_REC_BOX'] - (int)$value['YOR_DEL_BOX'];
				$teus = (int)$hariSebelumnya['YOR_TEUS'] + (int)$value['YOR_REC_TEUS'] - (int)$value['YOR_DEL_TEUS'];
			...
			$typ = round($total_ytp,2);
			$yordate = date('dmY',strtotime($value['YOR_DATE']));
