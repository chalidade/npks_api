...
class M_onchasis extends CI_Model {
	   ...
	   // $data = $this->db->select('A.REAL_YARD_REQ_NO, A.REAL_YARD_CONT, A.REAL_YARD_CONT_SIZE, A.REAL_YARD_CONT_TYPE, A.REAL_YARD_CONT_STATUS, B.YBC_YARD_ID, B.YBC_BLOCK_ID, B.YBC_SLOT, B.YBC_ROW, A.REAL_YARD_TIER')
		 // 					->from('TX_REAL_YARD A')
			...
			$data = $this->db->query("SELECT B.REQ_NO REAL_YARD_REQ_NO, B.REQUEST_TO, A.REQ_DTL_CONT REAL_YARD_CONT, A.REQ_DTL_CONT_STATUS REAL_YARD_CONT_STATUS, A.REQ_DTL_CONT_SIZE REAL_YARD_CONT_SIZE, A.REQ_DTL_CONT_TYPE REAL_YARD_CONT_TYPE, A.REQ_DTL_CONT_STATUS CONT_STATUS, 4 ACTIVITY, B.REQ_BRANCH_ID BRANCH, A.REQ_DTL_ACTIVE, E.OWNER_NAME OWNER
							...
							WHERE A.REQ_DTL_CONT = ? AND B.REQ_BRANCH_ID = ? ",$params)->row_array();
			...
			if(count($data) > 0){
				if($data['REQ_DTL_ACTIVE'] == 'Y'){
					...
					$message = 'Container sudah unstack dari lapangan';
	...
	function setOnChasis(){
		...
		if($req_to == 'TPK'){
			...
			$insertGATEIN = "INSERT INTO TX_GATE (GATE_NOREQ, GATE_CONT, GATE_CONT_STATUS, GATE_CONT_SIZE, GATE_CONT_TYPE, GATE_TRUCK_NO, GATE_ORIGIN, GATE_CREATE_DATE, GATE_STATUS, GATE_ACTIVITY, GATE_STACK_STATUS, GATE_FL_SEND, GATE_BRANCH_ID) VALUES
									 ('".$no_req."','".$cont."','".$cont_status."','".$cont_size."','".$cont_type."','PELINDO','".$req_to."', TO_DATE('".$gate_date_tpk."','MM/DD/YYYY HH24:MI:SS'),'1', '4','1', '9', ".$branch_id.")";
			$resultHDR = $this->db->query($insertGATEIN);

			$insertGATEIN = "INSERT INTO TX_GATE (GATE_NOREQ, GATE_CONT,GATE_CONT_STATUS, GATE_CONT_SIZE, GATE_CONT_TYPE, GATE_TRUCK_NO, GATE_ORIGIN, GATE_CREATE_DATE, GATE_STATUS, GATE_ACTIVITY, GATE_STACK_STATUS, GATE_FL_SEND, GATE_BRANCH_ID) VALUES
									 ('".$no_req."','".$cont."','".$cont_status."','".$cont_size."','".$cont_type."','PELINDO','".$req_to."', TO_DATE('".$gate_date_tpk."','MM/DD/YYYY HH24:MI:SS'),'3', '4','2', '9', ".$branch_id.")";
			$resultHDR = $this->db->query($insertGATEIN);
	...
	public function getRealYardCont(){
			$cont = $this->input->get('cont');
			$branch = $this->input->get('branch');
			$size = $this->input->get('size');
			$real_yard = $this->db->query("SELECT * FROM (SELECT REAL_YARD_ID, REAL_YARD_YBC_ID YBC_ID, REAL_YARD_TIER TIER, MAX(REAL_YARD_ID) over () as MAX_ID FROM TX_REAL_YARD WHERE REAL_YARD_BRANCH_ID = $branch AND REAL_YARD_CONT = '".$cont."' AND REAL_YARD_USED = 1) T
										WHERE T.REAL_YARD_ID = T.MAX_ID")->row_array();
			if($size == 40){
			$real_yard40 = $this->db->query("SELECT * FROM (SELECT REAL_YARD_ID, REAL_YARD_YBC_ID YBC_ID, REAL_YARD_TIER TIER, MIN(REAL_YARD_ID) over () as MAX_ID FROM TX_REAL_YARD WHERE REAL_YARD_BRANCH_ID = $branch AND REAL_YARD_CONT = '".$cont."' AND REAL_YARD_USED = 1) T
										WHERE T.REAL_YARD_ID = T.MAX_ID")->row_array();
			}


			return array('REAL_YARD' => $real_yard, 'REAL_YARD40' => $real_yard40);
		}
