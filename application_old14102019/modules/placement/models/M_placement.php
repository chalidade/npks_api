...
class M_placement extends CI_Model {
	...
	function setPlacement(){
	 ...
	 //cek jika contianer sudah stacking
		$check_cont_inyard = $this->db->where('REAL_YARD_CONT',$cont)->where('REAL_YARD_USED',1)->from('TX_REAL_YARD')->count_all_results();
		if($check_cont_inyard > 0){
			$message = 'Container sudah stacking dilapangan';
			$success = false;
