...
class M_gateContainer extends CI_Model {
  ...
  function setGateInContainer() {
	...
	function setGateOutContainer(){
		...
		echo "SELECT * FROM (SELECT REAL_YARD_ID, REAL_YARD_YBC_ID YBC_ID, REAL_YARD_TIER TIER, MAX(REAL_YARD_ID) over () as MAX_ID FROM TX_REAL_YARD WHERE REAL_YARD_BRANCH_ID = $branch_id AND REAL_YARD_CONT = '".$cont."' AND REAL_YARD_USED = 1) T
									WHERE T.REAL_YARD_ID = T.MAX_ID";
