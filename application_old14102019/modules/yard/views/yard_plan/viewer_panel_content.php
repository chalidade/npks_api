		...
		<tr align="center" valign="top">
			<td align="center" valign="middle" style="padding-left: 2px; padding-right: 2px;">
				<ol id="selectable_<?=$tab_id?>">
					...
					?>
								<li class="ui-plan-default" ybc_id="<?=$ybc_id[$p]?>" index="<?=$i?>" slot="<?=$slot_[$p]?>" row="<?=$row_[$p]?>" tier="<?=$tier_[$p]?>" title="<?=$title[$p]?>" owner="<?=$owner[$p]?>" block_id="<?=$block_id[$p]?>" orientation="<?=$orientation[$p]?>" position="<?=$position[$p]?>"
								<?php if (($i%$m) == 0){ $j++;?>
              ...
              <?php } ?>> <div style="margin-top:-3px;"></div></li>
					...
					?>
								<li class="ui-stacking-default" ybc_id="<?=$ybc_id[$p]?>" index="<?=$i?>" slot="<?=$slot_[$p]?>" row="<?=$row_[$p]?>" tier="<?=$tier_[$p]?>" title="<?=$title[$p]?>" owner="<?=$owner[$p]?>" block_id="<?=$block_id[$p]?>" orientation="<?=$orientation[$p]?>" position="<?=$position[$p]?>"
								<?php if (($i%$m) == 0){ $j++;?>
								style="clear: both; border-bottom: solid #6dadd6; border-right: solid #6dadd6;border-width:0.15em;"
								<?php }else{ ?>
								style="border-bottom: solid #6dadd6; border-right: solid #6dadd6;border-width:0.15em;"
