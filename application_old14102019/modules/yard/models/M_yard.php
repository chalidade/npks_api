...
class M_yard extends CI_Model {
  ...
  public function set_block_bl_v($yard_id, $block_id, $arrCell, $row, $slot){
	...
	$query 		= "SELECT A.YBC_YARD_ID ,A.YBC_CELL_ID, B.BLOCK_ID, B.BLOCK_NAME, B.BLOCK_POSITION, B.BLOCK_ORIENTATION, B.BLOCK_TIER, A.YBC_ROW, A.YBC_SLOT,
							MAX(C.YP_STATUS) FLAG_STATUS,SUM(DECODE(NVL(C.YP_STATUS,0),0,0,1)) JML_TAKEN, 0 PLACEMENT,0 JML_PLACEMENT, B.BLOCK_ROW, B.BLOCK_SLOT, D.YARD_HEIGHT, D.YARD_WIDTH, A.YBC_ID, E.CAT_HDR_NAME OWNER_NAME
              ...
              JOIN TM_YARD D ON B.BLOCK_YARD_ID = D.YARD_ID
							...
							GROUP BY A.YBC_YARD_ID ,A.YBC_CELL_ID, B.BLOCK_ID, B.BLOCK_NAME, B.BLOCK_POSITION, B.BLOCK_ORIENTATION, B.BLOCK_TIER, A.YBC_ROW, A.YBC_SLOT, B.BLOCK_ROW, B.BLOCK_SLOT, D.YARD_HEIGHT, D.YARD_WIDTH, A.YBC_ID, E.CAT_HDR_NAME
	...
	$xml_str = "<yard>".$width_str.$height_str.$stack_str.$plan_str.$taken_str.$placement_str.$slot_str.$row_str.$tier_str.$title_str.$block_id_str.$orientation_str.$position_str.$label_str.$label_text_str.$ybc_id_str.$label_owner."</yard>";
...
function getPlanCategory(){
	$branch_id = $this->session->USER_BRANCH;
	$query = strtoupper($this->input->get('query'));
	$params = array('BRANCH_ID' => $branch_id);
	$filter = "";
	if($query){
		$filter = "AND UPPER(CAT_HDR_NAME) LIKE '%".$this->db->escape_like_str($query)."%' ESCAPE '!'";
	}
...
public function get_yard_stacking($id_yard, $block, $slot, $ybc_id){
  ...
  // )Z")->result_array();
	...
	$query2 = $this->db->query("SELECT C.BLOCK_NAME, B.YBC_ROW, B.YBC_SLOT, A.REAL_YARD_TIER, A.REAL_YARD_CONT, A.REAL_YARD_CONT_SIZE, A.REAL_YARD_CONT_TYPE, A.REAL_YARD_CONT_STATUS, A.REAL_YARD_COMMODITY, A.REAL_YARD_STATUS, A.REAL_YARD_CREATE_DATE, A.REAL_YARD_YBC_ID, NVL(E.REQUEST_TO,'T') DELIVERY_TO
	FROM TX_REAL_YARD A
	JOIN TX_YARD_BLOCK_CELL B ON B.YBC_ID = A.REAL_YARD_YBC_ID
	JOIN TM_BLOCK C ON C.BLOCK_ID = B.YBC_BLOCK_ID
