...
class Client extends CI_Controller {
		        ...
		        if($valResponse != 0){
		            ...
		            while ($i < $totalData) {
		            	...
		            	$insert = "INSERT INTO TP_CONTAINER (CONT_NO,CONT_TYPE,CONT_SIZE,CONT_STATUS,BLOCK,SLOT,\"ROW\",TIER,NO_REQ,STACK_DATE,OWNER_CODE, OWNER_NAME, CONSIGNEE_NAME, CONSIGNEE_ADDRESS, NPWP )
												VALUES ('".$CONT_NO."','".$CONT_TYPE."','".$CONT_SIZE."','".$CONT_STATUS."','".$BLOCK."','".$SLOT."', '".$ROW."', '".$TIER."', '".$NO_REQ."',TO_DATE('".$STACK_DATE."','MM/DD/YYYY HH24:MI:SS'), '".$OWNER_CODE."', '".$OWNER_NAME."', '".$CONSIGNEE_NAME."', '".$CONSIGNEE_ADDRESS."', '".$NPWP."')";
	...
	function getReceivingNoreq($noreq){
        ...
        if($valResponse != 0){
	        ...
	        $this->db->query($insertServices);
	        echo "getReceiving -> Sukses Insert | ".$ServiceID." | ".date('Y-m-d H:i:s')."<br>\n";
		}else{
        	echo "getReceiving -> ".$xml['document']['_c']['URresponse']['_v']." | ".date('Y-m-d H:i:s')."<br>\n";
		}
	}
	...
	function getStuffing($branch = false){
        ...
        if($valResponse != 0){
			$insertServices = "INSERT INTO TX_SERVICES (SERVICES_ID, SERVICES_METHOD, SERVICES_REQ_XML, SERVICES_RESP_XML, SERVICES_STATUS)
								VALUES (".$ServiceID.",'".$method."','".$lastDateNota."',".$response.",'1')";
			$this->db->query($insertServices);
			echo "getStuffing -> Sukses Insert | ".$ServiceID." | ".date('Y-m-d H:i:s')."<br>\n";
		}else{
			echo "getStuffing -> ".$xml['document']['_c']['URresponse']['_v']." | ".date('Y-m-d H:i:s')."<br>\n";
		}
	}
	...
	function generateSetGateIn($branch = false){
    	...
    	$sqlgetIn = "SELECT GATE_ID, GATE_CONT AS NO_CONTAINER, GATE_NOREQ AS NO_REQUEST, GATE_TRUCK_NO AS NOPOL, GATE_CREATE_BY AS ID_USER, TO_CHAR(GATE_CREATE_DATE,'MM/DD/YYYY HH24:MI:SS') AS TGL_IN, GATE_CONT_STATUS AS STATUS, GATE_ORIGIN FROM TX_GATE  WHERE to_char(GATE_CREATE_DATE,'YYYYMMDD') = '20190101' AND GATE_FL_SEND = '0' AND GATE_ACTIVITY = '3' AND GATE_STATUS = '1' AND GATE_BRANCH_ID = ".$branch." ORDER BY GATE_CREATE_DATE ASC";
    ...
    function SetGateIn($branch = false){
        ...
        if($totalservice > 0){
        	foreach($resultservices->result_array() as $row){
				...
				//$aa = $row['SERVICES_REQ_XML']->read(2000000);

				$aa = '8383838883';

				$params = array('string0'=>'npks','string1'=>'12345','string2'=>$aa);
    ...
    function generatePlacement($branch = false){
    	...
    	$sqlgetIn = "SELECT A.REAL_YARD_ID, A.REAL_YARD_YBC_ID, B.YBC_SLOT, B.YBC_ROW, B.YBC_BLOCK_ID, A.REAL_YARD_TIER TIER, A.REAL_YARD_NO ID_YARD, A.REAL_YARD_CONT NO_CONTAINER, A.REAL_YARD_REQ_NO NO_REQUEST, A.REAL_YARD_BRANCH_ID, TO_CHAR(A.REAL_YARD_CREATE_DATE,'MM/DD/YYYY HH24:MI:SS') AS TGL_PLACEMENT, A.REAL_YARD_CREATE_BY AS ID_USER, A.REAL_YARD_CONT_STATUS CONT_STATUS
					...
					WHERE to_char(A.REAL_YARD_CREATE_DATE,'YYYYMMDD') = '20190101' AND A.REAL_YARD_STATUS = '1' AND A.REAL_YARD_ACTIVITY = '3' AND A.REAL_YARD_FL_SEND = '0' AND A.REAL_YARD_BRANCH_ID = ".$branch." AND A.REAL_YARD_REQ_NO IS NOT NULL ORDER BY A.REAL_YARD_CREATE_DATE ASC";
    	$resultservices = $this->db->query($sqlgetIn);
        $totalservice = $resultservices->num_rows();

			// $sqlgetIn = "SELECT A.REAL_YARD_ID, A.REAL_YARD_YBC_ID, B.YBC_SLOT, B.YBC_ROW, B.YBC_BLOCK_ID, A.REAL_YARD_TIER TIER, A.REAL_YARD_NO ID_YARD, A.REAL_YARD_CONT NO_CONTAINER, A.REAL_YARD_REQ_NO NO_REQUEST, A.REAL_YARD_BRANCH_ID, TO_CHAR(A.REAL_YARD_CREATE_DATE,'MM/DD/YYYY HH24:MI:SS') AS TGL_PLACEMENT, A.REAL_YARD_CREATE_BY AS ID_USER, A.REAL_YARD_CONT_STATUS CONT_STATUS, A.REAL_YARD_FL_SEND
			// 			FROM TX_REAL_YARD A
			// 			INNER JOIN TX_YARD_BLOCK_CELL B ON B.YBC_ID = A.REAL_YARD_YBC_ID
			// 			WHERE A.REAL_YARD_STATUS = '1' AND A.REAL_YARD_ACTIVITY = ".$branch." AND A.REAL_YARD_FL_SEND = '0' AND A.REAL_YARD_BRANCH_ID = 3
			// 			AND A.REAL_YARD_CONT IN (
			// 			select GATE_CONT from tx_gate where gate_truck_no is not null AND GATE_ACTIVITY = '3' AND GATE_STATUS = '1'
			// 			)";
    	//$resultservices = $this->db->query($sqlgetIn);
        $totalservice = $resultservices->num_rows();
