...
class M_container extends CI_Model {
	...
	public function getStuffingHistory($filter){
		...
		$sql = "SELECT * FROM
				(
					...
					TO_CHAR(A.STUFF_CREATE_DATE ,'DD/MM/YYYY HH24:MI') DATE_REQUEST,
					(SELECT TO_CHAR(REAL_STUFF_DATE,'DD/MM/YYYY HH24:MI') FROM TX_REAL_STUFF WHERE REAL_STUFF_CONT = B.STUFF_DTL_CONT AND REAL_STUFF_NOREQ = A.STUFF_NO AND REAL_STUFF_BRANCH_ID = A.STUFF_BRANCH_ID AND REAL_STUFF_STATUS = '1' ) DATE_START,
					(SELECT TO_CHAR(REAL_STUFF_DATE,'DD/MM/YYYY HH24:MI') FROM TX_REAL_STUFF WHERE REAL_STUFF_CONT = B.STUFF_DTL_CONT AND REAL_STUFF_NOREQ = A.STUFF_NO AND REAL_STUFF_BRANCH_ID = A.STUFF_BRANCH_ID AND REAL_STUFF_STATUS = '2' ) DATE_END,
					TO_CHAR(B.STUFF_DTL_END_STUFF_PLAN,'DD/MM/YYYY') PAID_TRUE,
    	...
    	$sqlTotal = "SELECT * FROM(SELECT B.STUFF_DTL_CONT CONTAINER_NUMBER, A.STUFF_NO REQUEST_NUMBER,
					TO_CHAR(A.STUFF_CREATE_DATE ,'DD/MM/YYYY HH24:MI') DATE_REQUEST,
					(SELECT TO_CHAR(REAL_STUFF_DATE,'DD/MM/YYYY HH24:MI') FROM TX_REAL_STUFF WHERE REAL_STUFF_CONT = B.STUFF_DTL_CONT AND REAL_STUFF_NOREQ = A.STUFF_NO AND REAL_STUFF_BRANCH_ID = A.STUFF_BRANCH_ID AND REAL_STUFF_STATUS = '1' ) DATE_START,
					(SELECT TO_CHAR(REAL_STUFF_DATE,'DD/MM/YYYY HH24:MI') FROM TX_REAL_STUFF WHERE REAL_STUFF_CONT = B.STUFF_DTL_CONT AND REAL_STUFF_NOREQ = A.STUFF_NO AND REAL_STUFF_BRANCH_ID = A.STUFF_BRANCH_ID AND REAL_STUFF_STATUS = '2' ) DATE_END,
					TO_CHAR(B.STUFF_DTL_END_STUFF_PLAN,'DD/MM/YYYY') PAID_TRUE,
	...
	public function getStrippingHistory($filter){
		...
		$sql = "SELECT * FROM (
					...
					TO_CHAR(A.STRIP_CREATE_DATE,'DD/MM/YYYY HH24:MI') DATE_REQUEST,
					(SELECT TO_CHAR(REAL_STRIP_DATE,'DD/MM/YYYY HH24:MI') FROM TX_REAL_STRIP WHERE REAL_STRIP_CONT = B.STRIP_DTL_CONT AND REAL_STRIP_NOREQ = A.STRIP_NO AND REAL_STRIP_BRANCH_ID = A.STRIP_BRANCH_ID AND REAL_STRIP_STATUS = '1' ) DATE_START,
					(SELECT TO_CHAR(REAL_STRIP_DATE,'DD/MM/YYYY HH24:MI') FROM TX_REAL_STRIP WHERE REAL_STRIP_CONT = B.STRIP_DTL_CONT AND REAL_STRIP_NOREQ = A.STRIP_NO AND REAL_STRIP_BRANCH_ID = A.STRIP_BRANCH_ID AND REAL_STRIP_STATUS = '2' ) DATE_END,
					TO_CHAR(B.STRIP_DTL_END_STRIP_PLAN,'DD/MM/YYYY') PAID_TRUE,
          C.CONSIGNEE_NAME EMKL
					FROM
					TX_REQ_STRIP_HDR A
		...
		$sqlTotal = "SELECT * FROM (SELECT B.STRIP_DTL_CONT CONTAINER_NUMBER, A.STRIP_NO REQUEST_NUMBER,
		TO_CHAR(A.STRIP_CREATE_DATE,'DD/MM/YYYY HH24:MI') DATE_REQUEST,
		(SELECT TO_CHAR(REAL_STRIP_DATE,'DD/MM/YYYY HH24:MI') FROM TX_REAL_STRIP WHERE REAL_STRIP_CONT = B.STRIP_DTL_CONT AND REAL_STRIP_NOREQ = A.STRIP_NO AND REAL_STRIP_BRANCH_ID = A.STRIP_BRANCH_ID AND REAL_STRIP_STATUS = '1' ) DATE_START,
		(SELECT TO_CHAR(REAL_STRIP_DATE,'DD/MM/YYYY HH24:MI') FROM TX_REAL_STRIP WHERE REAL_STRIP_CONT = B.STRIP_DTL_CONT AND REAL_STRIP_NOREQ = A.STRIP_NO AND REAL_STRIP_BRANCH_ID = A.STRIP_BRANCH_ID AND REAL_STRIP_STATUS = '2' ) DATE_END,
		TO_CHAR(B.STRIP_DTL_END_STRIP_PLAN,'DD/MM/YYYY') PAID_TRUE
    ...
    $response = $this->call_service($client, $method,$params);
		...
		if($response->respon == 1){
				...
				$get_max_counter = $this->db->select_max('HIST_COUNTER')->get('TH_HISTORY_CONTAINER');
				...
				if(count($cek_hist_cont) > 0){
					foreach ($cek_hist_cont as $val) {
						...
						if($activity == 1 AND $activity_name == 'GATE IN'){
								...
								//CEK TX_CMS
								$checkCMS = $this->db->where('CMS_NOREQ',$rec)->where('CMS_CONT',$cont_new)->where('CMS_BRANCH_ID',$branch_i)->from('TX_CMS')->count_all_results();
								if($checkCMS > 0){
									$this->db->query("UPDATE TX_CMS SET CMS_CONT ='".$cont_new."' WHERE CMS_CONT = '".$cont_old."' AND CMS_NOREQ = '".$rec."' AND CMS_BRANCH_ID =".$branch_i);
								}
						}
					...
					//END LOOP
					$this->db->query("UPDATE TH_HISTORY_CONTAINER SET HIST_CONT ='".$cont_new."' WHERE HIST_CONT = '".$cont_old."'");

					$this->db->query("UPDATE TM_CONTAINER SET CONTAINER_NO ='".$cont_new."' WHERE CONTAINER_NO = '".$cont_old."'");
