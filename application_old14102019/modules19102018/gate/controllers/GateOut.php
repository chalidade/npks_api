...
class GateOut extends CI_Controller {
  ...
  public function getNoTruckRePrint(){
    $filter = isset($_GET['query']) ? $_GET['query'] : 0;
    $data = $this->m_gateContainer->getNoTruckRePrint($filter);
    header('Content-Type: text/javascript');
    die(json_encode($data));

  }

  public function getNoTruckEngkelGateOut(){
    $filter = isset($_GET['query']) ? $_GET['query'] : 0;
    $data = $this->m_gateTruck->getTruckOut($filter);
    header('Content-Type: text/javascript');
    die(json_encode($data));
  ...
  public function setGateOutTruck(){
    if($id = $this->check_token()){
      if($id == $this->session->userdata('isId')){
        $data = $this->m_gateTruck->setGateOutTruck();
