...
<div class="container">
	<div>
		<div id="yor" style="width: 100%; height: 33.333;">
			...
			<div class="yorfont" style="margin-top:50px; margin-left:30px;">
				<table align="center">
					<tr>
						<td><div id="yorpercent" class="yorpercent" style="display:inline">0</div></td>
						<td><div class="odometer" id="percent" style="display:inline; margin-left:2px;">%</div></td>
					</tr>
				</table>
			</div>
			<div class="yorfont2">
				<table align="center">
					<tr>
						<td><div id="stacking" class="stacking" style="display:inline">0</div></td>
						<td><div class="odometer" id="slash">/</div></td>
						<td><div id="total" class="total" style="display:inline">0</div></td>
					</tr>
				</table>
			</div>
		</div>
								...
								success: function(point) {
									...
									receiving.setTitle(null, { text: 'TOTAL :' + total});
										...
										var socket = io.connect(Contants.getNodeServer());
									      socket.on('updateReceiving', function(branch) {
											if(branch == Cookies.get('branch')){
												requestDataRec();
											}
									      });
								},
						...
						$.ajax({
								...
								success: function(point) {
									...
									sp2.setTitle(null, { text: 'TOTAL :' + total});
										...
										var socket = io.connect(Contants.getNodeServer());
									      socket.on('updateSP2', function(branch) {
											  if(branch == Cookies.get('branch')){
												requestDataSp2();
											  }
									      });
					...
					$.ajax({
							...
							success: function(point) {
								...
								stuffing.setTitle(null, { text: 'TOTAL :' + total});
									...
									var socket = io.connect(Contants.getNodeServer());
									      socket.on('updateStuffingProcess', function(branch) {
											  if(branch == Cookies.get('branch')){
												requestDataStuffing();
											  }
									      });
				...
				$.ajax({
						...
						success: function(point) {
							...
							stripping.setTitle(null, { text: 'TOTAL :' + total});
								...
								var socket = io.connect(Contants.getNodeServer());
									      socket.on('updateStrippingProcess', function(branch) {
											  if(branch == Cookies.get('branch')){
				...
				$.ajax({
						...
						success: function(point) {
							...
							repo.setTitle(null, { text: 'TOTAL :' + total});
								...
								var socket = io.connect(Contants.getNodeServer());
									      socket.on('updateRepo', function(branch) {
											  if(branch == Cookies.get('branch')){
												requestDataRepo();
											  }
									      });
			...
			function requestDataYor() {
				$.ajax({
						...
						success: function(point) {
								...
								var socket = io.connect(Contants.getNodeServer());
									      socket.on('updateYardMonitoring', function(branch) {
											  if(branch == Cookies.get('branch')){
												requestDataYor();
											  }
									      });
