...
var block_dummy_<?=$tab_id?> 		= new Array();
			...
			buttons: [{
				...
				handler: function() {
					if (this.up('form').getForm().isValid()){
						if (SetBlock_<?=$tab_id?>(this.up('form').getForm().findField("block_name").getValue(),this.up('form').getForm().findField("num_tier").getValue(),this.up('form').getForm().findField("block_position").getValue(),this.up('form').getForm().findField("orientation").getValue(),this.up('form').getForm().findField("block_color").getValue(),this.up('form').getForm().findField("dummy").getValue())){
							win.close();
						}
					}
				}
...
function SetBlock_<?=$tab_id?>(name, tier, position, orientation, color, dummy){
	...
	if (p == 0){
		...
		block_dummy_<?=$tab_id?>[count_block_<?=$tab_id?>]	 = dummy;
		count_block_<?=$tab_id?>++;
	}else{
