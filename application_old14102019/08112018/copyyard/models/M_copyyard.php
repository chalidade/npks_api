...
class M_copyyard extends CI_Model {
	...
	function setCopyYard(){
		...
		$cekTier = $this->db->where('REAL_YARD_YBC_ID',$ybc_id)->where('REAL_YARD_TIER',$tier)->where('REAL_YARD_BRANCH_ID',$branch_id)->from('TX_REAL_YARD')->count_all_results();
		if($cekTier <= 0) {
