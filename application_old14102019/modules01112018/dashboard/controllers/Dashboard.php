...
class Dashboard extends CI_Controller {
	...
	public function getDataRecOutstanding(){
		header("Content-type: text/json");
		$data = $this->m_dashboard->getDataRecOutstanding();
		echo json_encode($data);
	}

	public function getDataSp2Outstanding(){
		header("Content-type: text/json");
		$data = $this->m_dashboard->getDataSp2Outstanding();
		echo json_encode($data);
	}

	public function getDataRepoOutstanding(){
		header("Content-type: text/json");
		$data = $this->m_dashboard->getDataRepoOutstanding();
		echo json_encode($data);
	}

	public function getDataStuffingOutstanding(){
		header("Content-type: text/json");
		$data = $this->m_dashboard->getDataStuffingOutstanding();
		echo json_encode($data);
	}

	public function getDataStrippingOutstanding(){
		header("Content-type: text/json");
		$data = $this->m_dashboard->getDataStrippingOutstanding();
		echo json_encode($data);
	}
