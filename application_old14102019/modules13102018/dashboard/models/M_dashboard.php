...
class M_dashboard extends CI_Model {
	...
	public function getDataSp2(){
		...
		ELSE
		'OUTSTANDING'
		END OUTSTANDING, A.REQUEST_STATUS REQ_STATUS, A.REQ_BRANCH_ID BRANCH_ID, A.REQUEST_TO
		...
		WHERE REQUEST_TO = 'DEPO' AND T.BRANCH_ID = $branch_id AND (T.GATE_OUT IS NULL OR REQ_STATUS != 2))";
		$data = $this->db->query($sql)->result();
		return $data;
	}
	...
	public function getDataRepo(){
		...
		JOIN TX_REQ_DELIVERY_DTL B ON B.REQ_HDR_ID = A.REQ_ID
		LEFT JOIN TX_GATE C ON C.GATE_CONT = B.REQ_DTL_CONT AND C.GATE_STATUS = 1 AND C.GATE_NOREQ = A.REQ_NO) T
		WHERE REQUEST_TO = 'TPK' AND T.BRANCH_ID = $branch_id AND (T.GATE_OUT IS NULL OR REQ_STATUS != 2))";
		$data = $this->db->query($sql)->result();
		return $data;
	}
