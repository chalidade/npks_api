...
class M_gateContainer extends CI_Model {
  ...
  function setGateInContainer(){
	...
	if($data2->num_rows() > 0){
		...
		$this->db->insert('TH_CANCELLED',array('CANCELLED_NOREQ' => $REQ, 'CANCELLED_NO_CONT' => $CONT, 'CANCELLED_CREATE_BY' => $user_id, 'CANCELLED_STATUS' => 'Y'));
	}
	//end cek
	...
	function getGateJobManager(){
		...
		if(!empty($_REQUEST['TRUCK_NO'])){
			...
			$query = "SELECT F.* FROM (SELECT T.*, ROWNUM r FROM (SELECT
	S.request_no,
	S.ACTIVITY,
	S.request_dtl_cont,
	S.request_consignee_id,
	S.consignee_name,
	S.gate_truck_no,
	S.gate_origin,
	S.ORIGIN_NAME,
	NVL( TO_CHAR( S.GATE_IN, 'DD/MM/YYYY HH24:MI' ), 'Belum Gate In' ) GATE_IN,
	NVL( TO_CHAR( S.GATE_OUT, 'DD/MM/YYYY HH24:MI' ), 'Belum Gate Out' ) GATE_OUT,
	TO_CHAR( TGL_REQ, 'DD/MM/YYYY HH24:MI' ) TGL_REQ,
	NVL( TO_CHAR( S.PAID_THRU, 'DD/MM/YYYY' ), '-' ) PAID_THRU,
	CONT_SIZE,
	CONT_TYPE,
	CONT_STATUS
FROM
		...
		FROM
			(
				SELECT
					...
					'Receiving' ACTIVITY,
					B.REQUEST_DTL_CONT,
					A.REQUEST_CONSIGNEE_ID,
					D.CONSIGNEE_NAME,
					C.GATE_TRUCK_NO,
					C.GATE_ORIGIN,
					E.REFF_NAME ORIGIN_NAME,
					C.GATE_CREATE_DATE GATE_IN,
					(
						SELECT
							X.GATE_CREATE_DATE
						FROM
							TX_GATE X
						WHERE
							X.GATE_NOREQ = C.GATE_NOREQ
							AND X.GATE_CONT = C.GATE_CONT
							AND X.GATE_STATUS = 3
					) AS GATE_OUT,
					A.REQUEST_RECEIVING_DATE TGL_REQ,
					NULL PAID_THRU,
					B.REQUEST_DTL_CONT_SIZE CONT_SIZE,
					B.REQUEST_DTL_CONT_TYPE CONT_TYPE,
					B.REQUEST_DTL_CONT_STATUS CONT_STATUS,
					A.REQUEST_STATUS
				FROM
					TX_REQ_RECEIVING_HDR A
				JOIN TX_REQ_RECEIVING_DTL B ON
					B.REQUEST_HDR_ID = A.REQUEST_ID
				LEFT JOIN TX_GATE C ON
					C.GATE_CONT = B.REQUEST_DTL_CONT
					AND C.GATE_STATUS = 1
					AND C.GATE_NOREQ = A.REQUEST_NO
				LEFT JOIN TM_CONSIGNEE D ON
					D.CONSIGNEE_ID = A.REQUEST_CONSIGNEE_ID
				LEFT JOIN TM_REFF E ON
					E.REFF_ID = C.GATE_ORIGIN
					AND E.REFF_TR_ID = 20
				WHERE
					--A.REQUEST_STATUS != 2 AND
					 A.REQUEST_BRANCH_ID = $branch_id
				ORDER BY
					A.REQUEST_ID DESC
			)AA WHERE (AA.GATE_IN IS NULL OR GATE_OUT IS NULL)  OR AA.REQUEST_STATUS !=2
	UNION ALL SELECT
			*
		FROM
			(
				SELECT
					A.REQ_NO REQUEST_NO,
					'Delivery' ACTIVITY,
					B.REQ_DTL_CONT REQUEST_DTL_CONT,
					A.REQ_CONSIGNEE_ID REQUEST_CONSIGNEE_ID,
					D.CONSIGNEE_NAME CONSIGNEE_NAME,
					C.GATE_TRUCK_NO GATE_TRUCK_NO,
					C.GATE_ORIGIN,
					E.REFF_NAME ORIGIN_NAME,
					C.GATE_CREATE_DATE GATE_IN,
					(
					...
					A.REQ_DELIVERY_DATE TGL_REQ,
					B.REQ_DTL_DEL_DATE PAID_THRU,
					B.REQ_DTL_CONT_SIZE CONT_SIZE,
					B.REQ_DTL_CONT_TYPE CONT_TYPE,
					B.REQ_DTL_CONT_STATUS CONT_STATUS,
					A.REQUEST_STATUS
				FROM
					TX_REQ_DELIVERY_HDR A
				JOIN TX_REQ_DELIVERY_DTL B ON
					B.REQ_HDR_ID = A.REQ_ID
				LEFT JOIN TX_GATE C ON
					C.GATE_CONT = B.REQ_DTL_CONT
					AND C.GATE_STATUS = 1
					AND C.GATE_NOREQ = A.REQ_NO
				LEFT JOIN TM_CONSIGNEE D ON
					D.CONSIGNEE_ID = A.REQ_CONSIGNEE_ID
				LEFT JOIN TM_REFF E ON
					E.REFF_ID = C.GATE_ORIGIN
					AND E.REFF_TR_ID = 20
				WHERE
					--A.REQUEST_STATUS !=2 AND
 				A.REQ_BRANCH_ID = $branch_id
				ORDER BY
					A.REQ_ID DESC
			)AA WHERE (AA.GATE_IN IS NULL OR GATE_OUT IS NULL)  OR AA.REQUEST_STATUS !=2
	) S
